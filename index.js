...
const DB_FILE = 'nebula_v1100_db.json';
...
// 💰 বাইনেন্স ব্যালেন্স দেখার ফাংশন
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            slots.forEach(async (sl) => {
                ...
                sl.curP = s.p;
                if (sl.status === 'WAITING' && s.p <= sl.buy) { sl.status = 'BOUGHT'; sendTG(`📥 *বাই সম্পন্ন:* ${sl.sym} \n💵 দাম: ${s.p}`, config.cid); }
                ...
                if (sl.status === 'BOUGHT') {
                    ...
                    if (drop >= 0.40 && sl.dca < 12) {
                        ...
                        if (order) {
                            ...
                            sendTG(`🛡️ *DCA Shield:* ${sl.sym} (L${sl.dca})`, config.cid);
                        }
                    }
                    if (s.p >= sl.sell) {
...
const server = http.createServer((req, res) => {
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 flex items-center min-h-screen"><div class="max-w-md mx-auto space-y-6 w-full text-center">
        ...
        getBinanceBalance(user).then(balance => {
            ...
            <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
                <div class="p-6 bg-slate-900 rounded-[2.5rem] border border-sky-500/40 flex justify-between items-center shadow-xl shadow-sky-500/10">
