...
// ==============================================
// 👑 QUANTUM master engine v55.0 - SPEED BOOSTER & BDT UI
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v55_final.json';
        ...
        for (let uid in cachedUsers) {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'SELL_PENDING') {
                    if (!sl.maxNetBDT || parseFloat(sl.netBDT) > sl.maxNetBDT) sl.maxNetBDT = parseFloat(sl.netBDT);
                    
                    // ⚡ SPEED BOOSTER: টার্গেটের ৯০% গিয়ে নামা শুরু করলে বা টার্গেট হিট করলে
                    let nearTarget = sl.targetP * 0.9995; // সামান্য বাফার
                    let isReversing = (sl.maxNetBDT - parseFloat(sl.netBDT)) > 0.05;

                    if ((ms.p >= sl.targetP || (ms.p >= nearTarget && isReversing)) && parseFloat(sl.netBDT) >= 0.50) {
                        let msgPrefix = ms.p >= sl.targetP ? "✅ <b>PROFIT!</b>" : "⚡ <b>FAST EXIT!</b>";
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v55 PRO</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    let bdtVal = s.active ? (s.netBDT >= 0 ? '৳'+s.netBDT : '-৳'+Math.abs(s.netBDT)) : '';
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '% ('+bdtVal+')' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Goal: \${(s.targetP || 0).toFixed(4)}</div><div class="text-green-400 font-black">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum v55</div></div>\` : ''}</div>\`;
