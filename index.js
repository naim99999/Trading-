...
// ==============================================
// 🛡️ Quantum AI Master v110.0 - THE MASTERMIND
// ==============================================
...
let market = {};
COINS.forEach(c => market[c.s] = { p: 0, lp: 0, history: [], low: 0, btcTrend: 0, trend: 0 });
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            let u = cachedUsers[uid]; let maxSl = parseInt(u.slots) || 1; let feeR = u.fMode === 'bnb' ? 0.00045 : 0.0005;
            if (!u.userSlots || u.userSlots.length !== maxSl) { u.userSlots = Array(maxSl).fill(null).map((_, i) => ({ id: i, active: false, sym: '', buy: 0, sell: 0, slP: 0, qty: 0, pnl: 0, curP: 0, dca: 0, totalCost: 0, be: false, status: 'IDLE', netBDT: 0 })); saveDB(); }
            if (!u.cooldowns) u.cooldowns = {};

            if (u.isAuto) {
                let btcT = market["BTCUSDT"]?.btcTrend || 0;
                u.tSpeed = btcT > 0.01 ? "fast" : (btcT < -0.1 ? "safe" : "normal");
            }
                    ...
                    if (ms.p < (Math.max(...ms.history) * dLim) && calculateRSI(ms.history) < rLim && ms.p > (ms.low * 1.0001)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                ms.low = 0; saveDB();
                                sendTG(`🚀 <b>ENTRY: #${sym}</b>\n💰 মার্জিন: $${mE.toFixed(4)}\n⚙️ মোড: ${u.tSpeed.toUpperCase()}`, u.cid);
                            }
                            break;
                        }
                    }
