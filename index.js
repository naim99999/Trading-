...
// ==============================================
// 👑 QUANTUM master engine v66.2 - FULL UI FIXED
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v66_stable.json';
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                ...
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => existing[i] || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, curP: 0, dca: 0, netBDT: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                sl.curP = ms.p;
                let netUSD = (parseFloat(sl.qty) * ms.p) - sl.totalCost - (sl.totalCost * feeR * 2);
                ...
                for (let coin of COINS) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        ...
                        if (sIdx !== -1) {
                            ...
                            u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: coin.s, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: (entryVal/u.lev), curP: m.p, dca: 0, pnl: 0, netBDT: 0 };
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/toggle-pause') { 
        ...
        if (u) { u.isPaused = !u.isPaused; saveDB(); } 
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v66.2 PRO</div></div></div>
