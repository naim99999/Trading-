...
async function getBinanceBalance(u) {
    ...
    try { const res = await axios.get(`https://fapi.binance.com/fapi/v2/account?timestamp=${ts}&signature=${sig}`, { headers: { 'X-MBX-APIKEY': u.api }, timeout: 5000 }); return parseFloat(res.data.totalWalletBalance).toFixed(2); } catch(e) { return "0.00"; }
}
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (u.userSlots.length !== aiSlots) {
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => u.userSlots[i] || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, entryTime: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'BUY_PENDING') {
                    if (ms.p <= sl.buy || u.mode === 'demo') {
                        ...
                        let targetVal = (sl.id % 2 === 0) ? Number(u.evenT) : Number(u.oddT);
                        let targetPrice = (sl.buy * (1 + (targetVal/100) + feeR)).toFixed(COINS.find(c => c.s === sl.sym).d);
                        if (await placeOrder(sl.sym, "SELL", sl.qty, u, targetPrice)) {
                            sl.status = 'SELL_PENDING'; sl.targetP = targetPrice; saveDB();
                        }
                    } else if ((Date.now() - sl.entryTime)/1000 > 60) {
                        // ❌ ক্যান্সেল হলে মার্জিন ফেরত
                ...
                if (sl.status === 'SELL_PENDING') {
                    if (ms.p >= sl.targetP || u.mode === 'demo') {
                        ...
                        // ✅ সমাধান: ক্যাপিটাল = মেইন + ব্যবহৃত মার্জিন + নিট লাভ
                        if(u.mode === 'demo') u.cap = Number(u.cap) + sl.marginUsed + netProfitUSD;
                        u.profit = (u.profit || 0) + netProfitUSD;
                        sendTG(`✅ <b>PROFIT! #${sl.sym}</b>\nBalance: $${u.cap.toFixed(2)}`, u.cid);
            ...
            if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < aiSlots) {
                for (let sym of Object.keys(market)) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === sym)) {
                        ...
                        if (sIdx !== -1 && walletBal > (mNeeded + 1)) {
                            if (await placeOrder(sym, "BUY", qty, u, limitPrice)) {
                                if(u.mode === 'demo') u.cap = Number(u.cap) - mNeeded;
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: sym, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: mNeeded, entryTime: Date.now() };
                                saveDB();
                            }
                        }
                    }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/data') {
        ...
        return res.end(JSON.stringify({ ...u, balance: Number(rawB).toFixed(2), btcPrice: btc.p.toFixed(2), btcTrend: btc.btcTrend.toFixed(2) }));
    }
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">Quantum AI Engine</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v29 PRO</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; d.userSlots.forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${s.active ? statusColor : 'text-zinc-700'} tracking-wider">\${s.active ? s.sym + ' ['+s.status+']' : 'Slot '+(i+1)+' Idle'}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Target: \${(s.targetP || 0).toFixed(4)}</div><div>Live: \${(marketData[s.sym] || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum PRO</div></div>\` : ''}</div>\`;
                }); document.getElementById('slotContainer').innerHTML = h; } catch(e) {} } 
                const marketData = {}; setInterval(updateData, 1000);
        </script></body></html>`);
    }
});
