...
// ==============================================
// 👑 QUANTUM APEX AI v42.0 - ULTIMATE SYNC FIXED
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v42_final.json';
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            // 🛠️ স্থায়ী স্লট লজিক: রিফ্রেশ দিলেও ডাটা মুছবে না
            ...
            if (!u.userSlots || u.userSlots.length !== aiSlotsCount) {
                ...
                u.userSlots = Array(aiSlotsCount).fill(null).map((_, i) => {
                    return existing[i] || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, curP: 0, dca: 0, pnl: 0, entryTime: 0 };
                });
                saveDB();
            }
            ...
            if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < (aiSlotsCount - 1)) {
                for (let sym of Object.keys(market)) {
                    if (u.userSlots.filter(s => s.active).length >= (aiSlotsCount - 1)) break;
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v42 PRO</div></div></div>
