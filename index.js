...
// ==============================================
// 🛡️ Quantum AI Master v400.0 - THE SNIPER BEAST
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'nebula_sovereign_hub.json';
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // ডাইনামিক রিকভারি DCA
                let dcaT = sl.dca === 0 ? -1.5 : -4.5;
                if ((u.slots - activeTrades) >= 1 && rawPnL <= -0.6 && sl.dca < 2) dcaT = -0.6; 
                    ...
                    if (m.rsi < rLim && m.p < (Math.max(...m.history) * dLim) && m.p > (m.low * 1.0001)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                saveDB(); sendTG(`🚀 <b>SNIPER ENTRY: #${sym}</b>\nUser: ${uid}\nMargin: $${mE.toFixed(4)}`, u.cid);
                            }
                            break;
                        }
                    }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div id="slotContainer" class="space-y-3"></div><a href="/reset-logout?id=${userId}" onclick="return confirm('লগ আউট করবেন?')" class="block w-full bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase shadow-xl">Exit Hub</a></div><script>
