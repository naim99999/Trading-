...
// ==========================================
// 🛡️ অ্যাডমিন মাস্টার কনফিগ (আপনার তথ্য প্রিসেট)
// ==========================================
...
// 🎯 ৪০টি কয়েন পুল (স্লট ফাঁকা থাকবে না)
const COINS = [
...
// 🚀 হাই-ভেলোসিটি ইঞ্জিন (Turbo Mode)
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            if (!config.isPaused && activeStatus && slotIdx !== -1 && s.trend >= 2 && s.p < avgP) {
                ...
                let canBuy = sameCoin.length === 0 || s.p < Math.min(...sameCoin.map(x => x.buy)) * 0.995;
                ...
                if (canBuy) {
                    ...
                    if (order) {
                        ...
                        sendTG(`📥 *বাই সম্পন্ন করা হয়েছে!* (Slot ${slotIdx+1})\nকয়েন: *${COINS.find(c=>c.s===msg.s).n}*\nদাম: ${buyP}`, config.cid);
                    }
                }
            }
        }
...
// 🌐 মাস্টার ড্যাশবোর্ড সার্ভার (Koyeb Port Fix)
const server = http.createServer((req, res) => {
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen"><div class="max-w-md mx-auto space-y-6 w-full text-center">
            <h1 class="text-5xl font-black text-sky-400 italic">QUANTUM</h1>
