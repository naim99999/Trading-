...
// ==============================================
// 👑 QUANTUM master engine v72.0 - ANTI-MINUS REPAIR
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v72_final.json';
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            // 🤖 AI SLOTS Setup
            let aiSlots = walletBal < 30 ? 1 : (walletBal < 150 ? 3 : 5); // ১০০০ ডলারেও ৫ স্লট রাখা নিরাপদ
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => ({ id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, curP: 0, dca: 0, maxP: 0, totalFeePaid: 0, lastDcaTime: 0 }));
                saveDB();
            }
                    ...
                    if (parseFloat(sl.netBDT) >= 0.50) {
                        ...
                        if (dropFromPeak > 0.04) {
                            ...
                            if (await placeOrder(sl.sym, "SELL", sl.qty, u)) {
                                ...
                                Object.assign(sl, { active: false, status: 'IDLE', sym: '', marginUsed: 0, totalCost: 0, dca: 0, isClosing: false, lastDcaTime: 0 }); saveDB();
                            } else sl.isClosing = false;
                        }
                    }
                    // 🛡️ RECOVERY DCA: ২ মিনিটের আগে ২য় বার কিনবে না (ব্যালেন্স প্রোটেকশন)
                        ...
                        if (ms.p > ms.ema7) {
                            ...
                            if (await placeOrder(sl.sym, "BUY", addQty, u)) {
                                ...
                                u.cap = parseFloat((u.cap - addM).toFixed(2));
            ...
            if (!u.isPaused && u.userSlots.filter(s => s.active).length < aiSlots) {
                for (let coin of COINS) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        let entryVal = (walletBal * u.lev) / aiSlots / 5; // মার্জিন আরও কমানো হলো
                        ...
                        if (parseFloat(qty) > 0 && await placeOrder(coin.s, "BUY", qty, u)) {
                            ...
                            u.cap = parseFloat((u.cap - mUsed).toFixed(2));
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v72 REPAIR</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '%' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Gain: ৳\${s.netBDT}</div><div class="text-green-400">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">v72 REPAIR</div></div>\` : ''}</div>\`;
