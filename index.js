...
// ==========================================
// 🛡️ Quantum AI - Master Intelligent DCA v39.0
// ==========================================
...
const COINS = [
    ...
    { s: "FILUSDT", n: "FIL", d: 3, qd: 1 }, { s: "ARBUSDT", n: "ARB", d: 4, qd: 1 },
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            userSlots[userId].forEach(async (sl) => {
                ...
                // ১. ডায়নামিক প্রফিট ট্রেলিং (লাভ তাড়া করা)
                let currentTargetPct = ((sl.sell - sl.buy) / sl.buy) * 100 * config.lev;
                if (rawPnL >= currentTargetPct * 0.9) {
                    sl.sell = sl.buy * (1 + (currentTargetPct + 0.1) / (100 * config.lev)); // টার্গেট বৃদ্ধি
                    sl.slP = sl.buy * (1 + (rawPnL - 0.1) / (100 * config.lev)); // শিল্ড বৃদ্ধি
                    sl.be = true;
                }

                // ২. ব্রেক-ইভেন শিল্ড এক্টিভেশন
                ...
                // ৪. ইন্টেলিজেন্ট প্রফিট ক্লোজিং
                if (s.p >= sl.sell) {
                    if (s.trend > 7) return; // স্ট্রং ট্রেন্ডে ওয়েট করো
                ...
                // ৫. শিল্ড হিট (Trailing Shield Hit)
                if (sl.be && s.p <= sl.slP) {
                    const gain = (sl.qty * s.p) - (sl.totalCost);
                    const netGain = gain * 0.9985;
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 flex items-center min-h-screen text-center uppercase font-black italic tracking-tighter"><div class="max-w-md mx-auto w-full space-y-6">
            ...
            <body class="bg-[#020617] text-white p-4 font-sans uppercase">
                <div class="max-w-xl mx-auto space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="p-5 bg-slate-900 rounded-[2.5rem] text-center border border-slate-800 shadow-xl"><p class="text-[9px] text-slate-500 font-bold mb-1 uppercase tracking-widest">Completed</p><p class="text-4xl font-black text-sky-400" id="countText">0</p></div>
                    </div>
                    ...
                    async function updateData() {
                        try {
                            ...
                            data.slots.forEach((s, i) => {
                                ...
                                html += \`<div class="p-5 bg-slate-900/50 rounded-3xl border border-zinc-800 mb-3 shadow-lg">
                                    ...
                                    <div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1 uppercase"><div>Entry: \${s.buy.toFixed(4)}</div><div class="text-right">Live: \text-sky-400 \${s.curP}</div><div class="text-orange-400">Recovery Mode</div><div class="text-right text-green-500">Target: \${s.sell.toFixed(4)}</div></div>\` : ''}
                                </div>\`;
                            });
...
server.listen(PORT, '0.0.0.0', () => { startGlobalEngine(); sendTG("🚀 *Quantum Final Intelligence Core Online!*", FIXED_CHAT_ID); });
