...
// ==============================================
// 👑 QUANTUM FAST PROFIT HUB v7.5 - LIVE READY
// ==============================================
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (currentTotalProfitBDT >= Number(u.targetBDT) && u.status !== 'COMPLETED') {
                u.isPaused = true; u.status = 'COMPLETED'; saveDB();
                sendTG(`🎯 <b>TARGET COMPLETED!</b>\nTotal Growth: ৳${currentTotalProfitBDT.toFixed(2)}\n<i>Bot is now paused. No new entries.</i>`, u.cid);
            }

            if (btcT < -0.18 && !u.isPaused) { u.isPaused = true; u.sysPaused = true; saveDB(); }
            else if (u.sysPaused && btcT > -0.02) { u.isPaused = false; u.sysPaused = false; saveDB(); }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.netBDT >= minP) {
                    ...
                    if (await placeOrder(sl.sym, "SELL", sl.qty, u)) {
                        ...
                        // একুরেট ব্যালেন্স আপডেট (মার্জিন + প্রফিট)
                        if(u.mode === 'demo') u.cap = Number(u.cap) + sl.marginCost + gainUSD;
                        
                        sendTG(`✅ <b>PROFIT: #${sl.sym}</b>\nThis Trade: ৳${sl.netBDT.toFixed(2)}\nHub Total Profit: ৳${(u.profit * 124).toFixed(2)}\nTotal Hub Balance: $${Number(u.cap).toFixed(2)}`, u.cid);
                        Object.assign(sl, { active: false, sym: '', isClosing: false, pnl: 0, netBDT: 0, marginCost: 0, dca: 0, totalCost: 0 }); saveDB();
                    } else sl.isClosing = false;
                }

                let dcaStep = -4.0; 
                let nextDcaPnl = (sl.dca + 1) * dcaStep;
                if (sl.pnl <= nextDcaPnl && sl.dca < 5 && (sl.marginCost * 1.5) < u.cap * 0.95) {
                    let dQty = (parseFloat(sl.qty) * 0.9).toFixed(COINS.find(c => c.s === sl.sym).qd);
                    if (await placeOrder(sl.sym, "BUY", dQty, u)) {
                        ...
                        sl.dca++; 
                        sl.marginCost += addM; // মার্জিন কস্ট আপডেট
                        saveDB();
                        sendTG(`🌀 <b>DCA LEVEL ${sl.dca}: #${sl.sym}</b>\nAdded Margin: $${addM.toFixed(2)}`, u.cid);
                    }
                }
            });
            ...
            if (!u.isPaused && activeTrades < u.slots && u.status !== 'COMPLETED') {
                for (let sym of Object.keys(market)) {
                    ...
                    if (m.rsi < 35 && m.ema7 > m.ema25 && !u.userSlots.some(x => x.active && x.sym === sym)) {
                        let tV = (u.cap * u.lev) / u.slots / 2.8; 
                        ...
                        if (sIdx !== -1 && await placeOrder(sym, "BUY", qty, u)) {
                            ...
                            activeTrades++; saveDB(); sendTG(`🚀 <b>HUB ENTRY: #${sym}</b>\nMargin: $${marginNeeded.toFixed(2)}`, u.cid);
                        }
                    }
                }
            }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">BTC Market Pulse</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ APEX HUB ACTIVE</div></div></div>
