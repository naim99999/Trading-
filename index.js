    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            if (!config.isPaused && activeStatus && slotIdx !== -1 && s.trend >= 2 && s.p < avgPrice) {
                ...
                let canBuy = sameCoinPositions.length === 0 || s.p < Math.min(...sameCoinPositions.map(x => x.buy)) * 0.993;
...
const server = http.createServer((req, res) => {
    ...
    // মেইন লিঙ্কে ঢুকলে এখন রেজিস্ট্রেশন ফর্ম দেখাবে (Solution to your screenshot)
    if (!userId || !db[userId]) {
        ...
        <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
            <div class="p-6 bg-slate-900 rounded-[2rem] border border-sky-500/40 shadow-xl">
                <div class="flex justify-between items-center mb-2">
                    <div><h2 class="text-3xl font-black italic underline decoration-sky-600 underline-offset-8">${userId.toUpperCase()}</h2><p class="text-[9px] text-sky-400 font-black uppercase mt-4 tracking-widest">● ENGINE ACTIVE</p></div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="p-6 bg-slate-900 rounded-[2rem] border border-slate-800 relative overflow-hidden shadow-2xl">
                    <p class="text-[10px] text-slate-500 uppercase font-black">TOTAL PROFIT (USD)</p><p class="text-4xl font-bold text-green-400 mt-2">$${user.profit.toFixed(2)}</p><div class="absolute right-6 top-10 text-3xl opacity-15">💲</div>
                </div>
                <div class="p-6 bg-slate-900 rounded-[2rem] border border-slate-800 relative overflow-hidden shadow-2xl">
                    <p class="text-[10px] text-slate-500 uppercase font-black">SUCCESS TRADES</p><p class="text-4xl font-bold text-sky-400 mt-2">${user.count}</p><div class="absolute right-6 top-10 text-3xl opacity-15">💼</div>
                </div>
            </div>

            <div class="bg-zinc-900/50 p-6 rounded-[2rem] border border-zinc-800 flex justify-between items-center shadow-lg">
                <span class="text-xs font-bold uppercase text-slate-400 italic">Trade Status</span>
                <button onclick="location.href='/toggle-trade?id=${userId}'" class="px-6 py-2 rounded-full font-black text-[10px] uppercase transition ${user.isPaused ? 'bg-red-500/20 text-red-500' : 'bg-green-500/20 text-green-400 border border-green-500'}">${user.isPaused ? 'PAUSED' : 'RUNNING'}</button>
            </div>
