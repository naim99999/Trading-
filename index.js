...
const DB_FILE = 'nebula_rebound_db.json';
...
const COINS = [
    ...
    { s: "DOTUSDT", n: "DOT", d: 3, qd: 1 }, { s: "BCHUSDT", n: "BCH", d: 2, qd: 3 }
];
        ...
        for (let userId in allUsers) {
            ...
            slots.forEach(async (sl) => {
                ...
                if (sl.status === 'WAITING' && s.p <= sl.buy) {
                    ...
                    // ৩টি প্রোটেকশন অর্ডার বাইন্যান্সে পাঠানো
                    ...
                    sl.mid = await sendTG(`📥 *বাই সম্পন্ন করা হয়েছে!* \n📌 স্লট: #${sl.id+1} \n💰 কয়েন: *${sl.sym.replace('USDT','')}* \n💵 দাম: ${s.p} \n🎯 টার্গেট: ${sl.sell} \n🛡️ DCA 1: ${sl.dca1}`, config.cid);
                }
            ...
            // ৪-টিক এন্ট্রি লজিক (The Oracle Sniper)
            const slotIdx = slots.findIndex(sl => !sl.active);
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 4 && s.p < avgP) {
                const sameCoin = slots.filter(sl => sl.active && sl.sym === msg.s);
                // একই কয়েন হলে অন্তত ৩% গ্যাপ লাগবে
                if (sameCoin.length === 0 || s.p < Math.min(...sameCoin.map(x => x.buy)) * 0.97) {
                    const coin = COINS.find(c => c.s === msg.s);
                    const buyP = (s.p * 0.9998).toFixed(coin.d); 
                    const sellP = (parseFloat(buyP) * 1.0012).toFixed(coin.d);
                    const d1P = (parseFloat(buyP) * 0.985).toFixed(coin.d); // DCA 1.5% down
                    const d2P = (parseFloat(buyP) * 0.965).toFixed(coin.d); // DCA 3.5% down
...
const server = http.createServer((req, res) => {
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen text-center"><div class="max-w-md mx-auto space-y-6 w-full">
            <h1 class="text-5xl font-black text-sky-400 italic underline decoration-sky-600 uppercase">Quantum Portal</h1>
                    ...
                    ${slots.map((s,i) => {
                        ...
                        return `<div class="p-4 bg-black/40 rounded-2xl border border-zinc-800/50">
                            <div class="flex justify-between items-center mb-1">
                                ...
                                <div class="text-right">${s.active ? `<span class="text-xs font-bold ${s.pnl>=0?'text-green-500':'text-red-400'}">${s.pnl.toFixed(1)}%</span>` : '<span class="text-[9px] text-zinc-700 font-black tracking-widest">Searching...</span>'}</div>
                            </div>
                            ${s.active && s.status === 'BOUGHT' ? `
                                <div class="grid grid-cols-2 gap-x-2 text-[8px] text-slate-500 uppercase font-bold mt-1">
                                    ...
                                    <span class="text-red-400">DCA 1: ${s.dca1.toFixed(cD)}</span><span class="text-right text-green-500">Target: ${s.sell.toFixed(cD)}</span>
