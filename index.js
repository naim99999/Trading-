...
// 🎯 হাই-ভলিউম কয়েন পুল (৫০টি কয়েন)
const COINS = [
...
// 💰 লাইভ ব্যালেন্স চেক (ড্যাশবোর্ডের বক্স থেকে প্রাপ্ত এপিআই দিয়ে)
...
const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/admin-act' && url.searchParams.get('pass') === ADMIN_PASS) {
        saveUser(url.searchParams.get('user'), { status: 'active', expiry: new Date(Date.now() + (parseFloat(url.searchParams.get('hours')) * 60 * 60 * 1000)).toISOString() });
        ...
        getBinanceBalance(user).then(balance => {
            ...
            <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
                <div class="p-6 bg-slate-900 rounded-[2rem] border border-sky-500/40 shadow-xl shadow-sky-500/10">
                    ...
                    ${active ? `<p class="text-[9px] text-green-400 font-bold uppercase mt-2">● SYSTEM ONLINE</p>` : `<p class="text-[9px] text-red-500 font-bold uppercase mt-2">● EXPIRED</p>`}
                </div>
                ...
                <div class="p-6 bg-zinc-900/50 rounded-[2.5rem] border border-zinc-800 space-y-3 shadow-inner">
                    ${slots.map((s,i) => {
                        ...
                        return `<div class="p-4 bg-black/40 rounded-2xl border border-zinc-800/50 flex justify-between items-center transition-all ${s.active ? 'border-sky-500/20 shadow-lg' : ''}"><div><span class="text-[9px] font-bold text-slate-600 uppercase italic">Slot ${i+1}</span><p class="text-sm font-black ${s.active ? 'text-sky-400' : 'text-zinc-800'}">${s.active ? s.sym.replace('USDT','') : 'IDLE'}</p></div><div class="text-right">${s.active ? `<span class="text-xs font-bold ${s.pnl>=0?'text-green-500':'text-red-400'}">${s.pnl.toFixed(2)}% PNL</span>` : '<span class="text-[9px] text-zinc-700 font-black tracking-widest uppercase text-xs">Scanning</span>'}</div></div>${s.active && s.status === 'BOUGHT' ? `<div class="progress-bar"><div class="progress-fill" style="width: ${progress}%"></div></div>` : ''}`;
                    }).join('')}
                </div>
