...
// ==========================================
// 🛡️ অ্যাডমিন কনফিগারেশন (অপরিবর্তিত)
// ==========================================
...
// 🎯 ৫০টি হাই-ভলিউম কয়েন পুল (স্লট কখনো ফাঁকা থাকবে না)
const COINS = [
    ...
    { s: "APTUSDT", n: "APT", d: 3, qd: 1 }, { s: "SUIUSDT", n: "SUI", d: 4, qd: 1 }, { s: "OPUSDT", n: "OP", d: 4, qd: 1 },
    { s: "TIAUSDT", n: "TIA", d: 3, qd: 1 }, { s: "ARBUSDT", n: "ARB", d: 4, qd: 1 }, { s: "INJUSDT", n: "INJ", d: 3, qd: 2 },
    { s: "LTCUSDT", n: "LTC", d: 2, qd: 3 }, { s: "LINKUSDT", n: "LINK", d: 3, qd: 2 }, { s: "FILUSDT", n: "FIL", d: 3, qd: 1 },
    { s: "MATICUSDT", n: "MATIC", d: 4, qd: 0 }, { s: "BCHUSDT", n: "BCH", d: 2, qd: 3 }, { s: "ICPUSDT", n: "ICP", d: 3, qd: 1 },
    { s: "FTMUSDT", n: "FTM", d: 4, qd: 0 }, { s: "GRTUSDT", n: "GRT", d: 5, qd: 0 }, { s: "GALAUSDT", n: "GALA", d: 5, qd: 0 },
    { s: "AAVEUSDT", n: "AAVE", d: 2, qd: 3 }, { s: "STXUSDT", n: "STX", d: 4, qd: 0 }, { s: "THETAUSDT", n: "THETA", d: 3, qd: 1 }
];
...
async function startGlobalEngine() {
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            // ২. ইনস্ট্যান্ট এন্ট্রি (সুইচ চেক সহ)
            const slotIdx = slots.findIndex(sl => !sl.active);
            const canTrade = config.isPaused === false || config.isPaused === "false";

            if (canTrade && activeStatus && slotIdx !== -1 && s.trend >= 1 && s.p < avgP) {
                const sameCoin = slots.filter(sl => sl.active && sl.sym === msg.s);
                let canBuyCross = sameCoin.length === 0 || s.p < Math.min(...sameCoin.map(x => x.buy)) * 0.994;
...
const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/toggle-trade') {
        if(db[userId]) { 
            db[userId].isPaused = !db[userId].isPaused; 
            saveUser(userId, db[userId]); 
        }
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen text-center"><div class="max-w-md mx-auto space-y-6 w-full">
            <h1 class="text-5xl font-black text-sky-400 italic italic tracking-tighter uppercase">Quantum Master</h1>
        ...
        <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
            <div class="p-6 bg-slate-900 rounded-[2rem] border border-sky-500/40 shadow-xl shadow-sky-500/10">
                ...
                ${isAdmin ? `<p class="text-[9px] text-yellow-500 font-bold uppercase mt-1 animate-pulse">👑 Admin Access</p>` : (active ? `<p class="text-[9px] text-green-400 font-bold uppercase mt-1">Status: Active (${timeLeft.toFixed(1)}h left)</p>` : `<p class="text-[9px] text-red-500 font-bold uppercase mt-1">Expired</p>`)}
            </div>
