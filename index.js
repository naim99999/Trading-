...
// ==============================================
// 🛡️ Quantum AI Master v1000.5 - ULTRA SAFE EDITION
// ==============================================
...
// --- ⚙️ SAFETY SETTINGS ---
const HARD_STOP_LOSS = -15; // ১৫% লস হলে অটো ক্লোজ (লিকুইডেশন থেকে বাঁচাবে)
const BTC_PANIC_DROP = -1.5; // BTC ১.৫% এর বেশি পড়লে সব ট্রেড ফোর্স ক্লোজ
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            } else if (u.sysPaused && btcT > -0.05 && u.status !== 'COMPLETED') {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // 🔴 [PROTECTION 1] Hard Stop Loss
                if (sl.pnl <= HARD_STOP_LOSS) {
                    sl.isClosing = true;
                    if (await placeOrder(sl.sym, "SELL", sl.qty, u)) {
                        let loss = sl.netBDT / 124;
                        u.profit = Number(u.profit || 0) + loss;
                        sendTG(`🆘 <b>STOP LOSS HIT: #${sl.sym}</b>\nLoss: ৳${sl.netBDT.toFixed(2)}`, u.cid);
                        Object.assign(sl, { active: false, status: 'IDLE', isClosing: false });
                        saveDB(); return;
                    } else { sl.isClosing = false; }
                }

                // 🟠 [PROTECTION 2] BTC Panic Exit
                if (btcT <= BTC_PANIC_DROP) {
                    sl.isClosing = true;
                    if (await placeOrder(sl.sym, "SELL", sl.qty, u)) {
                        u.profit = Number(u.profit || 0) + (sl.netBDT / 124);
                        sendTG(`⚠️ <b>MARKET CRASH EXIT: #${sl.sym}</b>`, u.cid);
                        Object.assign(sl, { active: false, status: 'IDLE', isClosing: false });
                        saveDB(); return;
                    } else { sl.isClosing = false; }
                }

                // সেল লজিক (Profit Take)
                let minP = u.isPaused ? 0.20 : 1.00;
                let dropTrigger = sl.maxNetBDT * 0.90; // ১০% প্রফিট ড্রপ করলে সেল (Trailing)
                ...
                // 🟡 [PROTECTION 3] Safe DCA Logic
                let dcaT = sl.dca === 0 ? -2.5 : -5.0; // DCA গ্যাপ বাড়ানো হয়েছে
                if (rawPnL <= dcaT && sl.dca < 3 && ms.rsi < 30) { // RSI কম থাকলে তবেই DCA করবে
            ...
            // --- 🚀 এন্ট্রি লজিক (আরো ফিল্টারড) ---
            if (!u.isPaused && activeTrades < u.slots && u.status !== 'COMPLETED' && btcT > -0.05) {
                for (let sym of Object.keys(market)) {
                    if (activeTrades >= u.slots) break;
                    const m = market[sym]; 
                    if (m.p === 0 || m.history.length < 20) continue;
                    
                    // এন্ট্রি শর্ত: RSI < 35 (Oversold) এবং মার্কেট একটু রিকভারি শুরু করলে
                    if (m.rsi < 35 && m.p > (m.low * 1.001)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            let tV = (u.cap * u.lev) / u.slots / 10;
...
// HTTP Server and UI remains same, but recommended leverage logic updated below
const server = http.createServer(async (req, res) => {
    // ... (rest of the server code) ...
    // Note: I'm keeping the server logic similar but check the `sug()` function in HTML below
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    const url = new URL(req.url, `http://${req.headers.host}`); const userId = url.pathname.slice(1);
    
    // API and Toggle logic same as before...
    if (url.pathname === '/api/data') {
        ...
        return res.end(JSON.stringify({ ...u, balance: rawB, btcPrice: btc.p.toFixed(2), btcTrend: btc.btcTrend.toFixed(2), pulse: btc.btcTrend > 0.05 ? "BULLISH" : (btc.btcTrend < -0.1 ? "BEARISH" : "NEUTRAL") }));
    }
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">BTC Market Pulse</p><p class="text-[10px] font-black" id="pM">Syncing...</p></div><div class="px-3 py-2 bg-green-600/20 border border-green-500/50 rounded-lg text-[8px] font-black text-green-400">🛡️ SAFE MODE ACTIVE</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; d.userSlots.forEach((s, i) => { 
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${s.active ? 'text-sky-400' : 'text-zinc-700'}">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)}</span>\${s.active ? \`<span class="text-[11px] font-black \${s.pnl>=0?'text-green-500':'text-red-400'}">\${s.pnl.toFixed(2)}%</span>\` : ''}</div></div>\`;
