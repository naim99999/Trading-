...
const CONFIG = {
    ...
    coins: [
        ...
        { s: "NEARUSDT", n: "NEAR", d: 4, qd: 1 }, { s: "AVAXUSDT", n: "AVAX", d: 3, qd: 2 },
        { s: "LINKUSDT", n: "LINK", d: 3, qd: 2 }, { s: "DOTUSDT", n: "DOT", d: 3, qd: 1 }
    ]
};
...
let market = {};
CONFIG.coins.forEach(c => market[c.s] = { p: 0, lp: 0, history: [], mom: 0 });
let userSlots = {}; 
let lastReportMin = -1;
...
async function startGlobalEngine() {
    ...
    ws.on('message', async (data) => {
        ...
        if (s.p > s.lp) s.mom = Math.min(100, s.mom + 15); else s.mom = Math.max(0, s.mom - 15);
        ...
        for (let userId in allUsers) {
            ...
            if (!userSlots[userId]) userSlots[userId] = Array(5).fill(null).map((_, i) => ({ id: i, active: false, status: 'IDLE', sym: '', buy: 0, sell: 0, qty: 0, pnl: 0, lastBuy: 0, entryT: 0, curP: 0 }));
            let slots = userSlots[userId];

            slots.forEach(async (sl) => {
                if (!sl.active || sl.sym !== msg.s) return;
                sl.curP = s.p;
                if (sl.status === 'WAITING' && s.p <= sl.buy) { sl.status = 'BOUGHT'; sl.entryT = Date.now(); }

                if (sl.status === 'BOUGHT') {
                    sl.pnl = ((s.p - sl.buy) / sl.buy) * 100 * (config.lev || 50);

                    // 🎯 রিব্যালেন্সিং লজিক (আপনার আইডিয়া): অল্প লাভে সেল করে ডাম্প হওয়া কয়েন কেনা
                    const timeInTrade = (Date.now() - sl.entryT) / 60000;
                    if (sl.pnl >= 2.0 && timeInTrade > 10) { // ১০ মিনিটের বেশি ঝুলে থাকলে ২% লাভে সেল
                        sl.sell = s.p; 
                    }

                    if (s.p >= sl.sell) {
                        const gain = (sl.qty * sl.sell) - (sl.qty * sl.buy) - (sl.qty * sl.sell * CONFIG.netFee);
                        if (gain >= 0.01) {
            ...
            // এন্ট্রি লজিক (কমে কেনা)
            const slotIdx = slots.findIndex(sl => !sl.active);
            if (!config.isPaused && slotIdx !== -1 && s.p < avgP * 0.998) { // গড় দামের ০.২% নিচে না নামলে কিনবে না
                if (!slots.some(sl => sl.active && sl.sym === payload.s)) {
                    ...
                    if (order) slots[slotIdx] = { id: slotIdx, active: true, status: 'WAITING', sym: msg.s, buy: parseFloat(buyP), sell: parseFloat(sellP), qty: qty, pnl: 0, lastBuy: parseFloat(buyP), entryT: Date.now(), curP: s.p };
                }
            }
        }
    });
...
// 🌐 মাস্টার ড্যাশবোর্ড (Design Fixed)
const server = http.createServer((req, res) => {
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen text-center"><div class="max-w-md mx-auto space-y-6 w-full">
            <h1 class="text-5xl font-black text-sky-400 italic italic tracking-tighter uppercase underline decoration-sky-600">Quantum Master</h1>
            <form action="/register" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 text-left shadow-2xl border border-sky-500/10">
                <input name="id" placeholder="User ID" class="w-full bg-black p-4 rounded-2xl border border-slate-800 text-white outline-none focus:border-sky-500" required>
                ...
                <button class="w-full bg-sky-600 p-5 rounded-[2rem] font-black uppercase shadow-lg transition transform active:scale-95">Launch Engine</button>
            </form></div></body></html>`);
    } else {
        let user = db[userId];
