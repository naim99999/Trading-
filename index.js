...
// ==============================================
// 👑 QUANTUM APEX AI v39.0 - API STATUS & LIVE SYNC
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_final_v39.json';
...
// 🔍 API কানেক্টিভিটি চেক ফাংশন
async function getBinanceBalance(u) {
    if (!u || !u.api || u.mode === 'demo') return { bal: Number(u?.cap || 0).toFixed(2), status: "DEMO" };
    ...
    try { 
        ...
        return { bal: parseFloat(res.data.totalWalletBalance).toFixed(2), status: "CONNECTED" };
    } catch (e) { 
        return { bal: "0.00", status: "AUTH_ERROR" }; 
    }
}
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => ({ id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, entryTime: 0, orderId: null }));
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'BUY_PENDING') {
                    if (ms.p <= sl.buy || u.mode === 'demo') {
                        ...
                        if (sellOrder) {
                            sl.status = 'SELL_PENDING'; sl.targetP = targetPrice; sl.orderId = sellOrder.orderId;
                            sendTG(`🚀 <b>BUY HIT: #${sl.sym}</b>\nPrice: ${sl.buy}\nTarget: ${targetPrice} (${tVal}%)`, u.cid);
                            saveDB();
                        }
                    } else if ((Date.now() - sl.entryTime)/1000 > 60) {
                        await cancelOrder(sl.sym, sl.orderId, u);
                        if(u.mode === 'demo') u.cap = Number(u.cap) + sl.marginUsed;
                ...
                if (sl.status === 'SELL_PENDING') {
                    if (ms.p >= sl.targetP || u.mode === 'demo') {
                        ...
                        if(u.mode === 'demo') u.cap = Number(u.cap) + sl.marginUsed + netUSD;
                        u.profit = (Number(u.profit || 0)) + netUSD;
                        sendTG(`✅ <b>PROFIT! #${sl.sym}</b>\nGain: ৳${(netUSD * 124).toFixed(2)}\nWallet: $${u.cap || walletBal}`, u.cid);
                        Object.assign(sl, { active: false, status: 'IDLE', sym: '', marginUsed: 0, totalCost: 0, orderId: null }); saveDB();
                    }
                    return;
                }
            });

            if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < aiSlots) {
                for (let sym of Object.keys(market)) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === sym)) {
                        ...
                        if (sIdx !== -1 && walletBal > (mNeeded + 2)) {
                            let buyOrder = await placeOrder(sym, "BUY", qty, u, limitPrice);
                            if (buyOrder) {
                                if(u.mode === 'demo') u.cap = Number(u.cap) - mNeeded;
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: sym, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: mNeeded, entryTime: Date.now(), orderId: buyOrder.orderId };
                                saveDB();
                            }
                        }
                    }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">API Connection</p><p class="text-[10px] font-black" id="apiS">Checking...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v39</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' ['+s.status+']' : 'Slot '+(i+1)+' Idle'}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Target: \${(s.targetP || 0).toFixed(4)}</div><div class="text-green-400">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum PRO</div></div>\` : ''}</div>\`;
