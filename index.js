...
async function startGlobalEngine() {
    ...
    ws.on('message', async (data) => {
        ...
        if (s.p > s.lp) s.streak = Math.min(10, s.streak + 1); else if(s.p < s.lp) s.streak = 0;
        ...
        for (let userId in allUsers) {
            ...
            // ২. জিরো-সেকেন্ড স্নাইপার এন্ট্রি
...
const server = http.createServer((req, res) => {
    ...
    if (!userId || !users[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen text-center"><div class="max-w-md mx-auto space-y-6 w-full">
            <h1 class="text-5xl font-black text-sky-400 italic italic">QUANTUM PORTAL</h1>
            <p class="text-xs text-yellow-500 font-bold uppercase tracking-widest border border-yellow-500/20 inline-block px-4 py-1 rounded-full">৳${settings.hourlyPrice} / ঘণ্টা</p>
            <form action="/register" class="bg-slate-900 p-8 rounded-[3rem] space-y-4 text-left shadow-2xl">
                ...
                <select name="mode" class="w-full bg-black p-4 rounded-2xl border border-slate-800 text-white"><option value="live">Live Trading (API Required)</option><option value="demo">Demo Mode (Free)</option></select>
                ...
                <button class="w-full bg-sky-600 p-5 rounded-[2rem] font-black uppercase tracking-widest shadow-lg active:scale-95 transition">Launch Portal</button>
            </form>
        </div></body></html>`);
    } else {
        ...
        <body class="bg-[#020617] text-white p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
            <div class="p-6 bg-slate-900 rounded-[2.5rem] border ${active ? 'border-green-500/40' : 'border-red-500/30'} flex justify-between items-center shadow-xl">
                <div><h2 class="text-3xl font-black italic underline decoration-sky-600">${userId.toUpperCase()}</h2><p class="text-[10px] text-slate-500 uppercase tracking-widest mt-1">${user.mode === 'demo' ? '🟡 DEMO MODE' : '🔵 LIVE MODE'}</p></div>
                <div class="text-right">
                    <div class="text-xs font-bold text-slate-500 uppercase">মোট লাভ (৳)</div>
                    <div class="text-3xl font-black text-green-400">৳${(user.profit * 124).toFixed(0)}</div>
                </div>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div class="bg-zinc-900/80 p-5 rounded-3xl border border-zinc-800 text-center shadow-inner"><p class="text-[10px] text-slate-500 uppercase font-bold mb-1">Gain (USD)</p><p class="text-2xl font-bold text-green-400">$${user.profit.toFixed(2)}</p></div>
                <div class="bg-zinc-900/80 p-5 rounded-3xl border border-zinc-800 text-center shadow-inner"><p class="text-[10px] text-slate-500 uppercase font-bold mb-1">Trades</p><p class="text-2xl font-bold text-sky-400">${user.count}</p></div>
            </div>

            <div class="bg-zinc-900/50 p-6 rounded-[2rem] border border-zinc-800 space-y-2">
                ${slots.map((s,i) => `<div class="flex justify-between p-4 bg-black/40 rounded-2xl border border-zinc-800/50"><div><span class="text-[9px] font-bold text-slate-600 uppercase italic">Slot ${i+1}</span><p class="text-sm font-black ${s.active ? 'text-sky-400' : 'text-zinc-800'}">${s.active ? s.sym.replace('USDT','') : 'IDLE'}</p></div><div class="text-right">${s.active ? `<span class="text-xs font-bold ${s.pnl>=0?'text-green-500':'text-red-400'}">${s.pnl.toFixed(2)}%</span>` : '<span class="text-[9px] text-zinc-700 font-black tracking-widest uppercase">Scanning</span>'}</div></div>`).join('')}
            </div>

            <div class="p-6 bg-zinc-900/50 rounded-[2.5rem] border border-zinc-800 space-y-4 text-center">
                <div class="flex justify-between text-[10px] text-slate-500 uppercase font-bold px-2"><span>Capital: $${user.cap}</span><span>Lev: ${user.lev}x</span></div>
                <button onclick="if(confirm('Reset all statistics?')) location.href='/reset-now?id=${userId}'" class="w-full bg-red-600/10 text-red-500 py-3 rounded-2xl font-black uppercase text-[10px] border border-red-500/20 hover:bg-red-600 hover:text-white transition">রিসেট অল ডাটা</button>
            </div>
            <div class="text-center opacity-40"><button onclick="location.href='/'" class="text-[10px] text-slate-500 font-bold uppercase underline">Switch Account</button></div>
