...
// ==============================================
// 🛡️ Quantum AI Master v1000.4 - THE GRACEFUL EXIT (SL ENHANCED)
// ==============================================
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.pnl <= -15.0) {
                    ...
                    if (res) {
                        ...
                        sendTG(`🚨 <b>EMERGENCY SL TRIGGERED!</b>\nSymbol: #${sl.sym}\nLoss: ৳${sl.netBDT.toFixed(2)}\n<i>Trade closed to prevent liquidation.</i>`, u.cid);
                        if(u.mode === 'demo') u.cap = Number(u.cap) + loss + (sl.totalCost / u.lev);
                        Object.assign(sl, { active: false, status: 'IDLE', sym: '', isClosing: false, maxNetBDT: 0, pnl: 0, netBDT: 0 }); 
                        saveDB();
                        return; // Exit current slot logic
