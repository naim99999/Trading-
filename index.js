...
// ==============================================
// 🛡️ Quantum Master Hub v600.0 - THE SUPREME OVERSEER
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'nebula_supreme_hub.json';
                    ...
                    if (m.rsi < rLim && m.p < (Math.max(...m.history) * dLim) && m.p > (m.low * 1.0001)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                activeTrades++; saveDB(); sendTG(`🚀 <b>SUPREME ENTRY: #${sym}</b>\nMargin: $${mE.toFixed(4)}`, u.cid);
                            }
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/register') { 
        let q = url.searchParams; let id = q.get('id'), cap = Number(q.get('cap')), lev = Number(q.get('lev')), slots = Number(q.get('slots')), target = Number(q.get('target'));
    ...
    if (!userId || !cachedUsers[userId]) {
        res.end(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><style>@keyframes glow{0%{box-shadow:0 0 5px #0ea5e9;}50%{box-shadow:0 0 20px #0ea5e9;}100%{box-shadow:0 0 5px #0ea5e9;}}</style></head><body class="bg-[#020617] text-white p-6 flex items-center min-h-screen font-sans"><div class="max-w-md mx-auto w-full space-y-6 text-center"><h1 class="text-6xl font-black text-sky-400 italic">QUANTUM</h1><form action="/register" method="GET" class="bg-slate-900/80 backdrop-blur-xl p-8 rounded-[2.5rem] space-y-4 border border-slate-800 shadow-2xl"><input name="id" placeholder="Username" class="w-full bg-black/50 p-4 rounded-xl outline-none border border-slate-800 focus:border-sky-500 transition-all" required><div class="grid grid-cols-2 gap-2"><select name="mode" class="bg-black/50 p-4 rounded-xl border border-slate-800 outline-none"><option value="live">Live Trading</option><option value="demo">Demo Mode</option></select><select name="fmode" class="bg-black/50 p-4 rounded-xl border border-slate-800 outline-none"><option value="usdt">Fee: USDT</option><option value="bnb">Fee: BNB</option></select></div><input name="api" placeholder="Binance API Key" class="w-full bg-black/50 p-4 rounded-xl border border-slate-800 outline-none"><input name="sec" placeholder="Binance Secret" class="w-full bg-black/50 p-4 rounded-xl border border-slate-800 outline-none"><input name="cid" placeholder="Telegram Chat ID" class="w-full bg-black/50 p-4 rounded-xl border border-slate-800 outline-none"><div class="grid grid-cols-2 gap-2"><input id="capI" name="cap" type="number" placeholder="Capital $" class="bg-black/50 p-4 rounded-xl border border-slate-800 outline-none" oninput="sug()"><input name="target" type="number" placeholder="Target ৳" class="bg-black/50 p-4 rounded-xl border border-slate-800 outline-none"></div><div class="p-4 bg-sky-900/20 rounded-xl text-[10px] text-sky-400 font-mono" id="sBox">💡 SUGGESTION: INPUT CAPITAL...</div><div class="grid grid-cols-2 gap-2"><input name="lev" id="levI" type="number" placeholder="Leverage" class="bg-black/50 p-4 rounded-xl border border-slate-800 outline-none"><input name="slots" id="slotI" type="number" placeholder="Slots" class="bg-black/50 p-4 rounded-xl border border-slate-800 outline-none"></div><button type="submit" style="animation:glow 3s infinite;" class="w-full bg-sky-600 p-5 rounded-full font-black uppercase tracking-widest hover:bg-sky-500 transition-all">Launch Engine</button></form></div><script>function sug(){let c=document.getElementById('capI').value; if(c){let l=c<15?20:25; let s=c<20?1:(c<60?2:(c<100?3:5)); document.getElementById('sBox').innerText="💡 SUGGESTED: "+l+"x Leverage, "+s+" Slots."; document.getElementById('levI').value=l; document.getElementById('slotI').value=s;}}</script></body></html>`);
    } else {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">BTC Market Pulse</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ AUTO PROTECTION</div></div></div>
        <div class="p-6 bg-slate-900/80 rounded-[2.5rem] border-2 border-sky-500/30 text-center shadow-2xl"><p class="text-[10px] text-sky-400 font-bold mb-1 italic">Silent Wallet Balance</p><p class="text-5xl font-black text-white">$<span id="balanceText">0.00</span></p></div>
        <div class="grid grid-cols-2 gap-4 text-center"><div class="p-6 bg-slate-900/80 rounded-[2.5rem] border border-slate-800 shadow-xl"><p class="text-[9px] text-slate-500 font-bold mb-1">Growth (BDT)</p><p class="text-4xl font-black text-green-400">৳<span id="profitText">0</span></p><div class="w-full bg-black/50 h-1 rounded-full mt-3 overflow-hidden"><div id="targetBar" class="h-full bg-green-500 transition-all duration-1000" style="width:0%"></div></div></div><div class="p-6 bg-slate-900/80 rounded-[2.5rem] border border-slate-800 shadow-xl"><p class="text-[9px] text-slate-500 font-bold mb-1 italic">Target BDT</p><p class="text-4xl font-black text-sky-400">৳<span id="targetText">0</span></p><p class="text-[8px] text-slate-500 mt-2" id="percentText">0% Done</p></div></div>
        <div id="slotContainer" class="space-y-3"></div><a href="/reset-logout?id=${userId}" onclick="return confirm('লগ আউট করবেন?')" class="block w-full bg-slate-800/50 border border-slate-700 text-slate-500 py-5 rounded-full text-center text-[10px] font-black uppercase">Exit Supreme Hub</a></div><script>
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let curP = Number(d.profit || 0) * 124; document.getElementById('profitText').innerText = curP.toFixed(2);
                document.getElementById('targetText').innerText = d.targetBDT || "0";
                let progress = Math.min(100, (curP / Number(d.targetBDT)) * 100);
                document.getElementById('targetBar').style.width = progress + "%";
                document.getElementById('percentText').innerText = progress.toFixed(1) + "% Done";
