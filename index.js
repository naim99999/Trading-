...
// ==============================================
// 🛡️ Quantum AI v2000.2 - THE DUAL-FORCE APEX
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'nebula_dualforce_hub.json';
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            let totalSlots = Number(u.slots); 

            // ডুয়াল স্লট জেনারেশন (Frontline + Rescue)
            if (!u.userSlots || u.userSlots.length !== totalSlots) {
                u.userSlots = Array(totalSlots).fill(null).map((_, i) => ({ id: i, active: false, sym: '', buy: 0, sell: 0, slP: 0, qty: 0, pnl: 0, curP: 0, dca: 0, totalCost: 0, be: false, status: 'IDLE', netBDT: 0, maxNetBDT: 0, rescueReady: true }));
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                let minThreshold = (Number(u.cap) < 10) ? 0.40 : 1.00;
                
                if (sl.netBDT >= minThreshold && sl.netBDT <= (sl.maxNetBDT - 0.01)) {
                    ...
                    setTimeout(() => { Object.assign(sl, { active: false, status: 'IDLE', sym: '', isClosing: false, maxNetBDT: 0, dca: 0, rescueReady: true }); saveDB(); }, 1200);
                }

                // ডুয়াল ফোর্স DCA (পেছনের স্লটের শক্তি ব্যবহার)
                let dcaTrigger = sl.dca === 0 ? -1.2 : -4.5;
                if (rawPnL <= dcaTrigger && sl.rescueReady && (sl.totalCost/u.lev)*2 < u.cap*0.9) {
                    if (await placeOrder(sl.sym, "BUY", sl.qty, u)) {
                        ...
                        sl.buy = sl.totalCost / parseFloat(sl.qty); sl.dca++; sl.sell = sl.buy * 1.0030; sl.be = false; sl.maxNetBDT = 0;
                        if (sl.dca >= 2) sl.rescueReady = false; // ২ বার এর পর ব্যাকআপ শেষ
                        saveDB(); sendTG(`🌀 <b>RESCUE ACTIVE: #${sl.sym}</b>\nNew Avg: $${sl.buy.toFixed(4)}`, u.cid);
                    }
                }
            });
            ...
            if (!u.isPaused && activeCount < totalSlots && currentProfitBDT < Number(u.targetBDT)) {
                let rLim = u.tSpeed === 'fast' ? 70 : 50, dLim = u.tSpeed === 'fast' ? 0.9992 : 0.9975;
                for (let sym of Object.keys(market)) {
                    if (activeCount >= totalSlots) break;
                    const m = market[sym]; if (m.p === 0 || m.history.length < 10 || (u.cooldowns && u.cooldowns[sym] > Date.now())) continue;
                    if (m.rsi < rLim && m.p < (Math.max(...m.history) * dLim) && m.p > (m.low * 1.0001)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            // বাজেট ভাগ করার সময় ডুয়াল স্লট মাথায় রেখে ১/৪০ করা হয়েছে (সেফটি)
                            let tV = Math.max(5.1, (u.cap * u.lev) / totalSlots / 20), qty = (tV / m.p).toFixed(COINS.find(c => c.s === sym).qd);
                            ...
                            if (sIdx !== -1 && await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'TRADING', sym: sym, buy: m.p, sell: m.p * 1.0040, slP: 0, qty: qty, pnl: 0, curP: m.p, dca: 0, totalCost: (parseFloat(qty) * m.p), be: false, netBDT: -0.05, isClosing: false, maxNetBDT: 0, rescueReady: true };
                                activeCount++; saveDB(); sendTG(`🚀 <b>FRONTLINE ENTRY: #${sym}</b>`, u.cid);
                            }
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/register') { 
        ...
        cachedUsers[id] = { api: q.get('api'), sec: q.get('sec'), cid: q.get('cid'), cap: cap, lev: lev, slots: slots, targetBDT: target, mode: q.get('mode'), fMode: q.get('fmode'), profit: 0, count: 0, isPaused: false, status: 'ACTIVE', cdSec: 30, userSlots: [], cooldowns: {} };
    ...
    if (!userId || !cachedUsers[userId]) {
        res.end(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-[#020617] text-white p-6 flex items-center min-h-screen font-sans text-center"><div class="max-w-md mx-auto w-full space-y-6"><h1 class="text-7xl font-black text-sky-400 italic">APEX</h1><form action="/register" method="GET" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 border border-slate-800 text-left shadow-2xl"><input name="id" placeholder="Username" class="w-full bg-black p-4 rounded-xl outline-none" required><div class="grid grid-cols-2 gap-2"><select name="mode" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><option value="live">Live Trading</option><option value="demo">Demo Mode</option></select><select name="fmode" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><option value="usdt">Fee: USDT</option><option value="bnb">Fee: BNB</option></select></div><input name="api" placeholder="Binance API Key" class="w-full bg-black p-4 rounded-xl outline-none"><input name="sec" placeholder="Binance Secret" class="w-full bg-black p-4 rounded-xl outline-none"><input name="cid" placeholder="Telegram ID" class="w-full bg-black p-4 rounded-xl outline-none"><div class="grid grid-cols-2 gap-2"><input id="capI" name="cap" type="number" placeholder="Capital $" class="bg-black p-4 rounded-xl outline-none" oninput="sug()"><input name="target" type="number" placeholder="Target ৳" class="bg-black p-4 rounded-xl outline-none"></div><div class="grid grid-cols-2 gap-2"><input name="lev" id="levI" type="number" placeholder="Leverage" class="bg-black p-4 rounded-xl outline-none"><input name="slots" id="slotI" type="number" placeholder="Slots" class="bg-black p-4 rounded-xl outline-none"></div><button type="submit" class="w-full bg-sky-600 p-5 rounded-full font-black uppercase">Start Dual-Force</button></form></div><script>function sug(){let c=document.getElementById('capI').value; if(c){let l=c<15?20:25; let s=c<30?1:(c<80?2:3); document.getElementById('levI').value=l; document.getElementById('slotI').value=s;}}</script></body></html>`);
    } else {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">BTC Market Pulse</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ DUAL-FORCE</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><a href="/reset-logout?id=${userId}" onclick="return confirm('লগ আউট?')" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Exit Master Hub</a></div><script>
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; d.userSlots.forEach((s, i) => { let m = s.active ? Math.max(0, Math.min(100, ((s.curP - s.buy) / (s.sell - s.buy)) * 100)) : 0;
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${s.active ? 'text-sky-400' : 'text-zinc-700'} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Frontline Idle'}</span>\${s.active ? \`<span class="text-[11px] font-black \${s.netBDT>=0?'text-green-500':'text-red-400'}">\${s.pnl.toFixed(2)}% (৳\${s.netBDT.toFixed(2)})</span>\` : ''}</div>\${s.active ? \`<div class="w-full bg-black h-1.5 rounded-full overflow-hidden mb-4"><div class="h-full bg-sky-500 transition-all duration-1000" style="width: \${m}%"></div></div><div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right">Live: \${s.curP.toFixed(4)}</div><div class="text-indigo-400 italic text-center col-span-2 mt-1">\${s.rescueReady ? 'RESERVE FORCE READY' : 'RESERVE FORCE USED'}</div></div>\` : ''}</div>\`;
