...
// ==============================================
// 👑 QUANTUM AI MASTER v93.0 - LIVE FEED RESTORED
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = '/tmp/quantum_final_v93.json';
...
let market = {};
COINS.forEach(c => market[c.s] = { p: 0, history: [], ema7: 0, ema25: 0, rsi: 50, btcTrend: 0, lastV: 0, curV: 0 });
...
function startGlobalEngine() {
    // 🛠️ আরও শক্তিশালী কানেকশন পদ্ধতি
    ...
    ws.on('message', (data) => {
        try {
            ...
            if (market[msg.s]) {
                ...
                s.lastV = s.curV; s.curV = parseFloat(msg.q);
                if (msg.s === "BTCUSDT" && s.history.length > 10) s.btcTrend = ((s.p - s.history[0]) / s.history[0] * 100);
            }
        } catch(e) {}
    });
            ...
            if (parseFloat(sl.netBDT) >= 0.50) {
                ...
                if (ms.p > ms.ema7) {
                    ...
                    if (await placeOrder(sl.sym, "BUY", addQty, u)) {
                        ...
                        sl.totalFeePaid += (parseFloat(addQty) * ms.p * 0.0004);
            ...
            for (let coin of COINS) {
                ...
                if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                    ...
                    if (parseFloat(qty) > 0 && await placeOrder(coin.s, "BUY", qty, u)) {
                        ...
                        u.userSlots[sIdx] = { id: sIdx, active: true, status: 'ACTIVE', sym: coin.s, buy: m.p, qty: qty, totalCost: (parseFloat(qty) * m.p), marginUsed: mUsed, totalFeePaid: (parseFloat(qty) * m.p * 0.0004), curP: m.p, dca: 0, pnl: 0, maxP: m.p, isClosing: false, lastDcaTime: Date.now(), netBDT: 0 };
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900 rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v93 PRO</div></div></div>
