...
const DB_FILE = 'nebula_v700_db.json';
...
async function startGlobalEngine() {
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            // ২. ল্যাডার এন্ট্রি লজিক (আপনার আইডিয়া অনুযায়ী)
            const slotIdx = slots.findIndex(sl => !sl.active);
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 2 && s.p < avgPrice) {
                const coin = COINS.find(c => c.s === msg.s);
                
                // চেক: একই কয়েন অন্য স্লটে থাকলে অন্তত ১.০০% ড্রপ হতে হবে (স্লট রোটেশন সুরক্ষা)
                ...
                if(sameCoinPositions.length > 0) {
                    ...
                    if (s.p > minEntryPrice * 0.99) canBuy = false; // ১.০০% গ্যাপ
                }
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen"><div class="max-w-md mx-auto space-y-6 w-full text-center">
            <h1 class="text-5xl font-black text-sky-400 italic">QUANTUM</h1><form action="/register" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 text-left shadow-2xl border border-sky-500/10">
                <input name="id" placeholder="Username" class="w-full bg-black p-4 rounded-2xl border border-slate-800 outline-none text-white focus:border-sky-500" required>
        ...
        <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
            <div class="p-6 bg-slate-900 rounded-[2rem] border border-sky-500/40 shadow-xl shadow-sky-500/10">
                ...
                ${isAdmin ? `<p class="text-[9px] text-yellow-500 font-bold uppercase mt-1 animate-pulse">👑 Admin Access</p>` : `<p class="text-[9px] text-green-400 font-bold uppercase mt-1">Status: Active</p>`}
            </div>
