...
// ==========================================
// 🛡️ অ্যাডমিন কনফিগ (Central Preset)
// ==========================================
...
const DB_FILE = 'nebula_immortal_db.json';
...
async function getBinanceBalance(config) {
    ...
    try {
        const res = await axios.get(`https://fapi.binance.com/fapi/v2/account?timestamp=${ts}&signature=${signature}`, {
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            slots.forEach(async (sl) => {
                ...
                // বাই হওয়ার সাথে সাথে ৩টি অর্ডার বিন্যান্সে পাঠানো (অফলাইন প্রোটেকশন)
                if (sl.status === 'WAITING' && s.p <= sl.buy) {
                    ...
                    await placeOrder(sl.sym, "BUY", (sl.buy * 0.995).toFixed(cI.d), sl.qty, config, "LIMIT"); // DCA 1
                    await placeOrder(sl.sym, "BUY", (sl.buy * 0.988).toFixed(cI.d), (parseFloat(sl.qty) * 2).toFixed(cI.qd), config, "LIMIT"); // DCA 2
                    
                    sendTG(`📥 *বাই সম্পন্ন:* ${sl.sym}\nBinance-Side Protection Active.`, config.cid);
                }
                ...
                if (sl.status === 'BOUGHT') {
                    ...
                    if (s.p >= sl.sell) {
                        ...
                        sendTG(`🎉 *SOLD SUCCESS!* ${sl.sym} \n৳${(gain*124).toFixed(0)} লাভ হয়েছে।`, config.cid);
            ...
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 2 && s.p < avgP) {
                ...
                if (sameCoin.length === 0 || s.p < Math.min(...sameCoin.map(x => x.buy)) * 0.994) {
                    ...
                    if (order) slots[slotIdx] = { id: slotIdx, active: true, status: 'WAITING', sym: msg.s, buy: parseFloat(buyP), sell: parseFloat(sellP), qty: qty, pnl: 0, lastBuy: parseFloat(buyP), waitTime: Date.now(), curP: s.p };
                }
            }
        }
    });
...
// 🌐 মাস্টার ড্যাশবোর্ড UI (সব প্রিয় ডিজাইন সহ)
