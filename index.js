...
// ==============================================
// 🛡️ Quantum AI Master v77.0 - Ultimate Machine
// ==============================================
...
// ক্লিন এবং দ্রুত মুভমেন্ট হওয়া কয়েন লিস্ট
const COINS = [
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (rawPnL <= dcaT && sl.dca < 4 && (sl.totalCost / u.lev) * 2 < (u.cap * 0.85)) {
                    if (await placeOrder(sl.sym, "BUY", sl.qty, u)) {
                        ...
                        sendTG(`🌀 <b>DCA: #${sl.sym} (Level ${sl.dca})</b>\n💰 মার্জিন: $${stM.toFixed(4)}\n📉 মোট খরচ: $${(sl.totalCost / u.lev).toFixed(2)}`, u.cid);
                    }
                }
            ...
            if (!u.isPaused && sIdx !== -1 && !guardian) {
                let rLim = u.tSpeed === 'fast' ? 50 : (u.tSpeed === 'safe' ? 32 : 40);
                let dLim = u.tSpeed === 'fast' ? 0.9975 : (u.tSpeed === 'safe' ? 0.9940 : 0.9960);
                    ...
                    if (ms.p < (localHigh * dLim) && calculateRSI(ms.history) < rLim && ms.p > (ms.low * 1.0003)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                ms.low = 0; saveDB();
                                sendTG(`🚀 <b>ENTRY: #${sym}</b>\n💰 মার্জিন: $${mE.toFixed(4)}\n⚙️ মোড: ${u.isAuto ? 'অটো' : 'ম্যানুয়াল'} [${u.tSpeed.toUpperCase()}]`, u.cid);
                            }
                            break;
                        }
                    }
