...
// ==============================================
// 👑 QUANTUM master engine v64.1 - LEVERAGE SYNC
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v64_final.json';
...
// 🔥 অটো লেভারেজ সিঙ্ক ফাংশন
async function syncLeverage(sym, lev, u) {
    if (u.mode === 'demo') return true;
    const ts = Date.now(); let q = `symbol=${sym}&leverage=${lev}&timestamp=${ts}`;
    try { await axios.post(`https://fapi.binance.com/fapi/v1/leverage?${q}&signature=${sign(q, u.sec)}`, null, { headers: { 'X-MBX-APIKEY': u.api } }); return true; } 
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                ...
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => existing.find(x => x.id === i) || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, curP: 0, dca: 0, maxNetBDT: 0, entryTime: 0, totalFeePaid: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                let exitFee = currentVal * feeR;
                let netProfitUSD = (currentVal - sl.totalCost) - (sl.totalFeePaid + exitFee);
                ...
                if (sl.status === 'BUY_PENDING') {
                    if (ms.p <= sl.buy || u.mode === 'demo') {
                        ...
                        let sellOrder = await placeOrder(sl.sym, "SELL", sl.qty, u, targetPrice);
                        if (sellOrder) {
                            sl.status = 'SELL_PENDING'; sl.orderId = sellOrder.orderId; saveDB();
                ...
                if (sl.status === 'SELL_PENDING') {
                    if (parseFloat(sl.netBDT) > sl.maxNetBDT || !sl.maxNetBDT) sl.maxNetBDT = parseFloat(sl.netBDT);
                    let reversing = (sl.maxNetBDT - parseFloat(sl.netBDT)) > 0.05;

                    if ((ms.p >= sl.targetP || reversing) && parseFloat(sl.netBDT) >= 0.50) {
                        if(u.mode === 'demo') u.cap = Number(u.cap || 0) + Number(sl.marginUsed || 0) + netProfitUSD;
                        u.profit = (Number(u.profit || 0) + netProfitUSD);
                        sendTG(`✅ <b>PROFIT! #${sl.sym}</b>\nGain: <b>৳${sl.netBDT}</b>\nHub Total: <b>৳${(u.profit * 124).toFixed(2)}</b>`, u.cid);
                        Object.assign(sl, { active: false, status: 'IDLE', sym: '', marginUsed: 0, totalCost: 0, dca: 0, maxNetBDT: 0, totalFeePaid: 0 }); saveDB();
                    } 
                    else if (parseFloat(sl.pnl) < -4.0 && sl.dca < 6 && walletBal > (sl.marginUsed * 1.6)) {
                        if (ms.p > ms.ema7) {
                            ...
                            if (u.mode === 'demo') u.cap = Number(u.cap || 0) - addM;
                            ...
                            sl.marginUsed = Number(sl.marginUsed || 0) + addM; 
                            sl.dca++; sl.maxNetBDT = 0;
                            sl.targetP = (sl.buy * (1 + 0.006 + feeR)).toFixed(COINS.find(c => c.s === sl.sym).d);
                            saveDB(); sendTG(`🌀 <b>AI RECOVERY: #${sl.sym}</b>`, u.cid);
                        }
                    }
                    return;
                }
            ...
            if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < (aiSlots - 1)) {
                for (let coin of COINS) {
                    ...
                    if (walletBal < 30 && coin.r) continue; 
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        ...
                        if (sIdx !== -1) {
                            await syncLeverage(coin.s, u.lev, u); // 🔥 বাইন্যান্সে লেভারেজ অটো সেট হবে
                            let buyOrder = await placeOrder(coin.s, "BUY", qty, u, limitPrice);
                            if (buyOrder) {
                                if(u.mode === 'demo') u.cap = Number(u.cap || 0) - (entryVal/u.lev);
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: coin.s, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: (entryVal/u.lev), totalFeePaid: (entryVal*feeR), entryTime: Date.now(), curP: m.p, dca: 0, pnl: 0, maxNetBDT: 0, orderId: buyOrder.orderId };
                                saveDB();
                            }
                        }
                    }
...
// SERVER & UI (DASHBOARD SYNC FIXED)
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/register') { 
        ...
        cachedUsers[q.get('id')] = { api: q.get('api'), sec: q.get('sec'), cid: q.get('cid'), cap: Number(q.get('cap')), targetBDT: Number(q.get('target')), lev: Number(q.get('lev')), mode: q.get('mode'), fMode: q.get('fmode'), profit: 0, isPaused: false, status: 'ACTIVE', userSlots: [] };
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v64.1 PRO</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' ['+s.status+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '%' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Goal: \${(s.targetP || 0).toFixed(4)}</div><div class="text-green-400 font-black">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">DCA: \${s.dca}</div></div>\` : ''}</div>\`;
