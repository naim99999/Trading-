...
// ==============================================
// 👑 QUANTUM APEX AI v23.0 - LIMIT SNIPER & AUTO-CANCEL
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_apex_v23.json';
...
let market = {};
COINS.forEach(c => market[c.s] = { p: 0, lp: 0, history: [], ema7: 0, ema25: 0, rsi: 50 });
...
async function placeOrder(sym, side, qty, u, price = null) {
    ...
    try { return (await axios.post(`https://fapi.binance.com/fapi/v1/order?${q}&signature=${sign(q, u.sec)}`, null, { headers: { 'X-MBX-APIKEY': u.api } })).data; } 
    catch (e) { return null; }
}

async function cancelOrder(sym, orderId, u) {
    if (u.mode === 'demo') return true;
    const ts = Date.now(); let q = `symbol=${sym}&orderId=${orderId}&timestamp=${ts}`;
    try { await axios.delete(`https://fapi.binance.com/fapi/v1/order?${q}&signature=${sign(q, u.sec)}`, { headers: { 'X-MBX-APIKEY': u.api } }); return true; }
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            let aiSlots = walletBal < 30 ? 2 : (walletBal < 150 ? 4 : 6);
            if (u.userSlots.length !== aiSlots) {
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => u.userSlots[i] || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, pnl: 0, curP: 0, dca: 0, totalCost: 0, netBDT: 0, maxPnl: 0, entryTime: 0, orderId: null });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'PENDING') {
                    let waitTime = (Date.now() - sl.entryTime) / 1000;
                    if (ms.p <= sl.buy) {
                        sl.status = 'ACTIVE'; sl.entryTime = Date.now();
                        sendTG(`🎯 <b>LIMIT HIT: #${sl.sym}</b>\nOrder Executed at Discounted Price.`, u.cid);
                    } else if (waitTime > 30) { // ৩০ সেকেন্ডের মধ্যে হিট না হলে ক্যান্সেল
                        await cancelOrder(sl.sym, sl.orderId, u);
                        if (u.mode === 'demo') u.cap += (sl.totalCost / u.lev);
                        Object.assign(sl, { active: false, status: 'IDLE', sym: '', orderId: null }); saveDB();
                        sendTG(`⏳ <b>LIMIT CANCELLED: #${sl.sym}</b>\nPrice didn't hit. Slot freed.`, u.cid);
                    }
                    return;
                }

                sl.curP = ms.p;
                let totalTradeValue = (parseFloat(sl.qty) * ms.p) + sl.totalCost;
                let netProfitUSD = (parseFloat(sl.qty) * ms.p) - sl.totalCost - (totalTradeValue * feeR);
                sl.pnl = (netProfitUSD / (sl.totalCost / u.lev)) * 100;
                sl.netBDT = netProfitUSD * 124;

                // Aggressive Profit Lock
                if (sl.netBDT > 1) { 
                    if (!sl.maxPnl || sl.pnl > sl.maxPnl) sl.maxPnl = sl.pnl;
                    let dropLimit = (u.profit*124 >= u.targetBDT) ? 0.01 : 0.03;
                    if ((sl.maxPnl - sl.pnl) >= dropLimit || sl.pnl > 5.0) {
                        sl.isClosing = true;
                        if (await placeOrder(sl.sym, "SELL", sl.qty, u)) {
                            u.profit = (u.profit || 0) + netProfitUSD;
                            if(u.mode === 'demo') u.cap = Number(u.cap) + (sl.totalCost/u.lev) + netProfitUSD;
                            sendTG(`✅ <b>PROFIT: #${sl.sym}</b>\nNet: ৳${sl.netBDT.toFixed(2)}`, u.cid);
                            Object.assign(sl, { active: false, status: 'IDLE', sym: '', isClosing: false, pnl: 0, maxPnl: 0, dca: 0, totalCost: 0, entryTime: 0 }); saveDB();
                        } else sl.isClosing = false;
                    }
                }

                // Recovery DCA
                if (sl.pnl < -2.5 && sl.dca < 7 && (Date.now() - sl.entryTime)/1000 > 40) {
                    if (ms.p > ms.ema7 || sl.pnl < -15) {
                        let dQty = (parseFloat(sl.qty) * 1.3).toFixed(COINS.find(c => c.s === sl.sym).qd);
            ...
            // 🚀 AI LIMIT SNIPER (FEE REDUCTION)
            let activeCount = u.userSlots.filter(s => s.active).length;
            if (!u.isPaused && activeCount < (aiSlots - 1)) {
                for (let sym of Object.keys(market)) {
                    if (activeCount >= (aiSlots - 1)) break;
                    const m = market[sym]; if (m.p === 0 || m.history.length < 50) continue;

                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === sym)) {
                        let entryVal = (walletBal * u.lev) / aiSlots / 3.8;
                        let qty = (entryVal / m.p).toFixed(COINS.find(c => c.s === sym).qd);
                        let limitPrice = (m.p - (m.p * feeR)).toFixed(COINS.find(c => c.s === sym).d);

                        const sIdx = u.userSlots.findIndex(sl => !sl.active);
                        if (sIdx !== -1) {
                            let order = await placeOrder(sym, "BUY", qty, u, limitPrice);
                            if (order) {
                                if(u.mode === 'demo') u.cap -= (entryVal/u.lev);
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'PENDING', sym: sym, buy: parseFloat(limitPrice), qty: qty, pnl: 0, curP: m.p, dca: 0, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), netBDT: 0, maxPnl: 0, isClosing: false, entryTime: Date.now(), orderId: order.orderId };
                                saveDB(); sendTG(`⏳ <b>LIMIT PLACED: #${sym}</b>\nPrice: ${limitPrice}\nWaiting 30s for hit.`, u.cid);
                            }
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/data') {
        ...
        let btc = market["BTCUSDT"] || { p: 0 };
        return res.end(JSON.stringify({ ...u, balance: Number(rawB).toFixed(2), btcPrice: btc.p.toFixed(2) }));
    }
    ...
    if (url.pathname === '/register') { 
        ...
        cachedUsers[id] = { api: q.get('api'), sec: q.get('sec'), cid: q.get('cid'), cap: cap, lev: lev, targetBDT: target, mode: q.get('mode'), fMode: q.get('fmode'), profit: 0, isPaused: false, userSlots: [] };
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">Limit Sniper Engine</p><p class="text-[10px] font-black" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v23</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; d.userSlots.forEach((s, i) => { 
                    let statusColor = s.status === 'PENDING' ? 'text-orange-400' : 'text-sky-400';
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${s.active ? statusColor : 'text-zinc-700'} tracking-wider">\${s.active ? s.sym + ' ['+s.status+']' : 'Slot '+(i+1)+' Idle'}</span>\${s.status === 'ACTIVE' ? \`<span class="text-[11px] font-black \${s.pnl>=0?'text-green-500':'text-red-400'}">\${s.pnl.toFixed(2)}% (৳\${s.netBDT.toFixed(2)})</span>\` : ''}</div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Peak: \${(s.maxPnl || 0).toFixed(2)}%</div><div>Live: \${s.curP.toFixed(4)}</div><div class="text-right italic">Quantum v23</div></div>\` : ''}</div>\`;
