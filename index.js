...
// ==============================================
// 👑 QUANTUM MASTER v49.0 - DASHBOARD REAL-TIME FIX
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_final_v49.json';
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (!u.userSlots || u.userSlots.length !== aiSlotsCount) {
                ...
                u.userSlots = Array(aiSlotsCount).fill(null).map((_, i) => existing.find(x => x.id === i) || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, curP: 0, entryTime: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                sl.curP = ms.p; // স্লটে লাইভ প্রাইস সিঙ্ক
                ...
                if (sl.status === 'BUY_PENDING') {
                    if (ms.p <= sl.buy) {
                        ...
                        sl.targetP = parseFloat((sl.buy * (1 + (adjTP/100) + feeR)).toFixed(COINS.find(c => c.s === sl.sym).d));
            ...
            if (!u.isPaused && u.userSlots.filter(s => s.active).length < (aiSlotsCount - 1)) {
                for (let coin of COINS) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        ...
                        if (sIdx !== -1) {
                            ...
                            u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: coin.s, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: (entryVal/u.lev), entryTime: Date.now(), curP: m.p };
                            saveDB();
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div class="flex justify-between items-center"><div><p class="text-[8px] text-slate-500 font-bold">AI Status</p><p class="text-[10px] font-black" id="apiS">Checking...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v49 PRO</div></div></div>
