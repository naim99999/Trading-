...
// ==============================================
// 🛡️ Quantum AI Master v73.0 - The Invincible
// ==============================================
...
// ক্লিন কয়েন লিস্ট (আটকে থাকা ৪টি বাদ দেওয়া হয়েছে)
const COINS = [
...
let market = {};
COINS.forEach(c => market[c.s] = { p: 0, lp: 0, history: [], low: 0, vol: 0, btcTrend: 0, trend: 0, cooldown: 0 });
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // ০.০১% ট্রেইলিং লক
                if (rawPnL >= 0.35) {
                    let lock = rawPnL - 0.01; if (ms.trend >= 7) lock = rawPnL - 0.05;
                ...
                let tV = sl.totalCost + (parseFloat(sl.qty) * ms.p), exF = tV * feeR, nG = ((parseFloat(sl.qty) * ms.p) - sl.totalCost) - exF;
                if ((ms.p >= sl.sell || (sl.be && rawPnL <= sl.slP)) && (nG * 124) >= 1) {
                    sl.status = 'COOLING'; u.profit = Number(u.profit || 0) + nG; u.count++;
                    ...
                    ms.cooldown = Date.now() + 1800000; // ৩০ মিনিট কুলডাউন
            ...
            if (!u.isPaused && sIdx !== -1 && !guardian) {
                ...
                for (let sym of Object.keys(market)) {
                    const ms = market[sym]; if (ms.p === 0 || ms.history.length < 20 || Date.now() < ms.cooldown) continue;
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/data') {
        ...
        return res.end(JSON.stringify({ slots: u?.userSlots || [], profit: u ? (u.profit * 124).toFixed(2) : 0, count: u ? u.count : 0, isPaused: u?.isPaused || false, balance: (Number(rawB) - (u?.mode === 'demo' ? 0 : activeM)).toFixed(2), lev: u?.lev || 0, tSpeed: u?.tSpeed || 'normal', pulse: pS, btcVal: btcT.toFixed(2), isAuto: u?.isAuto || false, guardian: u?.userSlots?.some(s => s.active && s.dca >= 3) }));
    }
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900 rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold" id="gMsg">BTC Market Pulse</p><p class="text-[10px] font-black" id="pM">Syncing...</p></div><div class="flex gap-1"><button onclick="setSpeed('fast', false)" id="btn-fast" class="px-2 py-2 rounded-lg text-[8px] font-black border border-slate-800">⚡</button><button onclick="setSpeed('normal', false)" id="btn-normal" class="px-2 py-2 rounded-lg text-[8px] font-black border border-slate-800">⚖️</button><button onclick="setSpeed('safe', false)" id="btn-safe" class="px-2 py-2 rounded-lg text-[8px] font-black border border-slate-800">🛡️</button><button onclick="setSpeed('', true)" id="btn-auto" class="px-2 py-2 rounded-lg text-[8px] font-black border border-slate-800">🤖 AUTO</button></div></div></div>
        <div class="p-6 bg-slate-900 rounded-[2.5rem] border-2 border-sky-500/50 text-center shadow-2xl tracking-tighter"><p class="text-[10px] text-sky-400 font-bold mb-1 uppercase tracking-widest italic">Wallet Balance</p><p class="text-5xl font-black text-white">$<span id="balanceText">0.00</span></p><div class="mt-2 text-[10px] text-slate-500 font-bold">Leverage: <span id="levText">0</span>x</div></div><div class="grid grid-cols-2 gap-4 text-center"><div class="p-6 bg-slate-900 rounded-[2.5rem] border border-slate-800 shadow-xl"><p class="text-[9px] text-slate-500 font-bold mb-1">Growth (BDT)</p><p class="text-4xl font-black text-green-400">৳<span id="profitText">0</span></p></div><div class="p-6 bg-slate-900 rounded-[2.5rem] border border-slate-800 shadow-xl"><p class="text-[9px] text-slate-500 font-bold mb-1 uppercase tracking-widest font-black">Wins</p><p class="text-4xl font-black text-sky-400" id="countText">0</p></div></div><div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset?id=${userId}" onclick="return confirm('রিসেট করবেন?')" class="bg-red-900/20 border border-red-500/30 text-red-500 py-5 rounded-full text-center text-[10px] font-black">Reset</a></div><a href="/reset-logout?id=${userId}" onclick="return confirm('লগ আউট করবেন?')" class="block w-full bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black">Logout & Reset</a></div>
        <script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); document.getElementById('balanceText').innerText = d.balance; document.getElementById('profitText').innerText = d.profit; document.getElementById('countText').innerText = d.count; document.getElementById('levText').innerText = d.lev; const pM = document.getElementById('pM'); const pB = document.getElementById('pB'); const gM = document.getElementById('gMsg');
                ...
                ['fast', 'normal', 'safe'].forEach(m => { const b = document.getElementById('btn-'+m); if(b) b.className = (d.tSpeed === m) ? "px-2 py-2 rounded-lg text-[8px] font-black bg-sky-600 text-white" : "px-2 py-2 rounded-lg text-[8px] font-black border border-slate-800 text-slate-500"; });
                ...
                }); document.getElementById('slotContainer').innerHTML = h; } catch(e) {} } setInterval(updateData, 800);
        </script></body></html>`);
    }
});
