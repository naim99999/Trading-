...
// ==============================================
// 🛡️ Quantum AI Master v300.0 - SOVEREIGN MASTER
// ==============================================
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            // টার্গেট চেক ও এন্ট্রি
            let currentTotalBDT = Number(u.profit || 0) * 124;
            if (currentTotalBDT >= Number(u.targetBDT)) {
                u.status = 'FINISHED'; if(activeTrades === 0) u.status = 'COMPLETED'; saveDB();
                    ...
                    if (m.rsi < rLim && m.p < (Math.max(...m.history) * dLim) && m.p > (m.low * 1.0001)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'TRADING', sym: sym, buy: m.p, sell: m.p * 1.0035, slP: 0, qty: qty, pnl: 0, curP: m.p, dca: 0, totalCost: (parseFloat(qty) * m.p), be: false, netBDT: -0.10 };
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/register') { 
        let q = url.searchParams; let id = q.get('id'); let cap = parseFloat(q.get('cap')); let lev = parseInt(q.get('lev')); let slots = parseInt(q.get('slots'));
        cachedUsers[id] = { api: q.get('api'), sec: q.get('sec'), cid: q.get('cid'), cap: cap, lev: lev, slots: slots, targetBDT: parseFloat(q.get('target')), mode: q.get('mode'), fMode: q.get('fmode'), profit: 0, count: 0, isPaused: false, status: 'ACTIVE', cdSec: 30, userSlots: Array(slots).fill(null).map((_, i) => ({ id: i, active: false, sym: '', buy: 0, sell: 0, slP: 0, qty: 0, pnl: 0, curP: 0, dca: 0, totalCost: 0, be: false, status: 'IDLE', netBDT: 0 })), cooldowns: {} };
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div id="slotContainer" class="space-y-3"></div><a href="/reset-logout?id=${userId}" onclick="return confirm('লগ আউট করবেন?')" class="block w-full bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Exit Hub</a></div><script>
