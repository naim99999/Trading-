...
// ==============================================
// 🛡️ Quantum AI Master v900.0 - THE FINAL APEX
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'nebula_apex_hub.json';
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // ট্রেইলিং লকিং লজিক (সর্বোচ্চ থেকে ০.০১ টাকা কমলে সেল)
                if (sl.netBDT > (sl.maxNetBDT || 0)) {
                    sl.maxNetBDT = sl.netBDT; 
                    sl.be = true;
                }
                ...
                if (rawPnL <= dcaT && sl.dca < (u.cap < 10 ? 2 : 4) && (sl.totalCost/u.lev)*2 < u.cap*0.95) {
                    if (await placeOrder(sl.sym, "BUY", sl.qty, u)) {
                        ...
                        sl.buy = sl.totalCost / parseFloat(sl.qty); sl.dca++; sl.sell = sl.buy * 1.0030; sl.be = false; sl.maxNetBDT = 0; saveDB();
                    }
                }

                // ক্লোজিং লজিক (আপনার ১ পয়সা কমার শর্ত)
                let minP = u.isPaused ? 0.20 : (Number(u.cap) < 10 ? 0.50 : 1.00);
                let dropGapTrigger = sl.maxNetBDT - 0.01;

                if (sl.netBDT >= minP && sl.be && sl.netBDT <= dropGapTrigger) {
                    sl.isClosing = true; let finalGain = sl.netBDT / 124;
                    u.profit = Number(u.profit || 0) + finalGain;
                    if(u.mode === 'demo') u.cap = Number(u.cap) + finalGain + (sl.totalCost / u.lev);
                    sendTG(`✅ <b>APEX PROFIT: #${sl.sym}</b>\nNet: ৳${sl.netBDT.toFixed(2)}\nTotal: ৳${(u.profit * 124).toFixed(2)}`, u.cid);
                    ...
                    if (m.rsi < rLim && m.p < (Math.max(...m.history) * dLim) && m.p > (m.low * 1.0001)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (sIdx !== -1 && await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                activeTrades++; saveDB(); sendTG(`🚀 <b>APEX ENTRY: #${sym}</b>`, u.cid);
                            }
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/data') {
        ...
        return res.end(JSON.stringify({ ...u, balance: (Number(rawB) - (u?.mode === 'demo' ? 0 : activeM)).toFixed(2), btcPrice: btc.p.toFixed(2), btcTrend: btc.btcTrend.toFixed(2), pulse: pS }));
    }
    ...
    if (!userId || !cachedUsers[userId]) {
        res.end(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-[#020617] text-white p-6 flex items-center min-h-screen font-sans text-center"><div class="max-w-md mx-auto w-full space-y-6"><h1 class="text-7xl font-black text-sky-400 italic">QUANTUM</h1><form action="/register" method="GET" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 border border-slate-800 text-left shadow-2xl"><input name="id" placeholder="Username" class="w-full bg-black p-4 rounded-xl outline-none" required><div class="grid grid-cols-2 gap-2"><select name="mode" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><option value="live">Live Trading</option><option value="demo">Demo Mode</option></select><select name="fmode" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><option value="usdt">Fee: USDT</option><option value="bnb">Fee: BNB</option></select></div><input name="api" placeholder="Binance API Key" class="w-full bg-black p-4 rounded-xl outline-none"><input name="sec" placeholder="Binance Secret" class="w-full bg-black p-4 rounded-xl outline-none"><input name="cid" placeholder="Telegram Chat ID" class="w-full bg-black p-4 rounded-xl outline-none"><div class="grid grid-cols-2 gap-2"><input id="capI" name="cap" type="number" placeholder="Capital $" class="bg-black p-4 rounded-xl outline-none" oninput="sug()"><input name="target" type="number" placeholder="Target ৳" class="bg-black p-4 rounded-xl outline-none"></div><div class="p-4 bg-sky-900/30 rounded-xl text-[10px] text-sky-400 font-mono" id="sBox">💡 SUGGESTION: INPUT CAPITAL...</div><div class="grid grid-cols-2 gap-2"><input name="lev" id="levI" type="number" placeholder="Leverage" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><input name="slots" id="slotI" type="number" placeholder="Slots" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"></div><button type="submit" class="w-full bg-sky-600 p-5 rounded-full font-black uppercase shadow-xl">Initialize Machine</button></form></div><script>function sug(){let c=document.getElementById('capI').value; if(c){let l=c<15?20:25; let s=c<20?1:(c<60?2:(c<100?3:5)); document.getElementById('sBox').innerText="💡 SUGGESTED: "+l+"x Leverage, "+s+" Slots."; document.getElementById('levI').value=l; document.getElementById('slotI').value=s;}}</script></body></html>`);
    } else {
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" onclick="return confirm('লগ আউট করবেন?')" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase shadow-xl text-center">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                document.getElementById('targetBar').style.width = Math.min(100, (Number(d.profit || 0) * 124 / Number(d.targetBDT)) * 100) + "%";
