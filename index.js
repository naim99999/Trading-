...
// ==============================================
// 👑 QUANTUM AI-AUTOPILOT v54.0 - NO-INPUT PROFIT
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v54_autopilot.json';
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'BUY_PENDING') {
                    if (ms.p <= sl.buy || u.mode === 'demo') {
                        ...
                        // 🤖 AI AUTO TARGET: ০.৬০% বেস টার্গেট
                        let tVal = 0.60;
                ...
                if (sl.status === 'SELL_PENDING') {
                    ...
                    let netProfitUSD = (totalValue - sl.totalCost) - estFees;
                    let netBDT = netProfitUSD * 124;

                    // 🎯 PROFIT HUNTER (Min 0.50 BDT)
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v54 PRO</div></div></div>
