...
// ==============================================
// 👑 QUANTUM master engine v68.0 - NET 0.50 BDT GUARD
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v68_final.json';

let cachedUsers = {}; 
function loadDB() { try { if (fs.existsSync(DB_FILE)) cachedUsers = JSON.parse(fs.readFileSync(DB_FILE, 'utf8')); } catch(e) { cachedUsers = {}; } }
function saveDB() { try { fs.writeFileSync(DB_FILE, JSON.stringify(cachedUsers, null, 2)); } catch(e) { console.log("DB Save Error"); } }
loadDB();
...
async function setLeverage(sym, lev, u) {
    if (u.mode === 'demo') return true;
    const ts = Date.now(); let q = `symbol=${sym}&leverage=${lev}&timestamp=${ts}`;
    try { await axios.post(`https://fapi.binance.com/fapi/v1/leverage?${q}&signature=${sign(q, u.sec)}`, null, { headers: { 'X-MBX-APIKEY': u.api } }); return true; } catch (e) { return false; }
}
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            let walletBal = parseFloat((await getBinanceBalance(u)).bal);
            let feeR = u.fMode === 'bnb' ? 0.00018 : 0.0002;
            let isTargetDone = (Number(u.profit || 0) * 124) >= Number(u.targetBDT);

            if (isTargetDone && !u.isPaused) { u.isPaused = true; saveDB(); sendTG(`🎯 <b>TARGET COMPLETED!</b>`, u.cid); }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // 🧮 পিওর নিট প্রফিট ক্যালকুলেশন (এন্ট্রি ফি + এক্সিট ফি বাদ দিয়ে)
                ...
                let netBDT = netProfitUSD * 124;
                sl.pnl = (((ms.p - sl.buy) / sl.buy) * 100 * u.lev).toFixed(2);

                if (sl.status === 'ACTIVE') {
                    // 🔥 নিট লাভ ০.৫০ টাকা পার হলে হান্টিং শুরু
                    if (netBDT >= 0.50) {
                        if (ms.p > sl.maxP) sl.maxP = ms.p; // সর্বোচ্চ দাম রেকর্ড

                        let dropFromPeak = ((sl.maxP - ms.p) / sl.maxP) * 100;
                        // যদি শিখর থেকে ০.০৩% দাম কমে, তবে সেল
                        if (dropFromPeak > 0.03 || netBDT > 100) { 
                            sl.isClosing = true;
                    ...
                    // RECOVERY DCA
                    else if (parseFloat(sl.pnl) < -3.5 && sl.dca < 6 && walletBal > (sl.marginUsed * 1.6)) {
                        if (ms.p > ms.ema7) {
            ...
            if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < (aiSlots - 1)) {
                for (let coin of COINS) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        ...
                        if (sIdx !== -1 && await placeOrder(coin.s, "BUY", qty, u)) {
                            let mUsed = entryVal / u.lev;
                            await setLeverage(coin.s, u.lev, u);
                            if(u.mode === 'demo') u.cap = parseFloat((u.cap - mUsed).toFixed(2));
                            u.userSlots[sIdx] = { id: sIdx, active: true, status: 'ACTIVE', sym: coin.s, buy: m.p, qty: qty, totalCost: (parseFloat(qty) * m.p), marginUsed: mUsed, totalFeePaid: (entryVal * feeR), curP: m.p, dca: 0, pnl: 0, maxP: m.p };
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v68 PRO</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    let statusColor = s.active ? (s.status === 'BUY_PENDING' ? 'text-orange-400' : 'text-sky-400') : 'text-zinc-700';
                    let pnlColor = parseFloat(s.pnl) >= 0 ? 'text-green-400' : 'text-red-400';
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '%' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Peak: \${s.maxP.toFixed(4)}</div><div class="text-green-400 font-black">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum v68</div></div>\` : ''}</div>\`;
