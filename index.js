...
// ==========================================
// 🛡️ মাস্টার কনফিগ (Admin: naim1155 | Pass: 115510)
// ==========================================
...
// 🎯 এঞ্জিন স্টেট
let market = {};
CONFIG.coins.forEach(c => market[c.s] = { p: 0, lp: 0, trend: 0, history: [], mom: 0 });
let userSlots = {}; 
...
async function startGlobalEngine() {
    const streams = CONFIG.coins.map(c => `${c.s.toLowerCase()}@ticker`).join('/');
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            slots.forEach(async (sl) => {
                ...
                if (sl.status === 'BOUGHT') {
                    ...
                    if (drop >= 0.45 && sl.dca < 10) {
                        const order = await placeOrder(sl.sym, "BUY", s.p.toFixed(CONFIG.coins.find(c=>c.s===sl.sym).d), sl.qty, config);
                        if (order) {
                            sl.buy = (sl.buy + s.p) / 2; sl.qty = (parseFloat(sl.qty) * 2).toFixed(CONFIG.coins.find(c=>c.s===sl.sym).qd);
                            sl.sell = (sl.buy * 1.0006).toFixed(CONFIG.coins.find(c=>c.s===sl.sym).d); sl.dca++; sl.lastBuy = s.p;
                        }
                    }
                    ...
                    if (s.p >= sl.sell) {
                        ...
                        if (gain >= 0.01) {
                            ...
                            // প্রিয় টেলিগ্রাম মেসেজ ফরম্যাট (বাংলায় মোট টাকা সহ)
                            sendTG(`🎉 *SOLD SUCCESS!* ${sl.sym} \n💵 লাভ: ৳${(gain*CONFIG.bdt).toFixed(0)} \n📈 মোট: ৳${(config.profit*CONFIG.bdt).toFixed(0)}`, config.cid);
            ...
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 2 && s.p < avgP) {
                const coin = CONFIG.coins.find(c => c.s === payload.s);
                if (!slots.some(sl => sl.active && sl.sym === payload.s)) {
                    ...
                    if (order) slots[slotIdx] = { id: slotIdx, active: true, status: 'WAITING', sym: payload.s, buy: parseFloat(buyP), sell: parseFloat(sellP), qty: qty, dca: 0, lastBuy: parseFloat(buyP), waitTime: Date.now(), curP: s.p };
                }
            }
        }
    });
...
const server = http.createServer((req, res) => {
    ...
    if (!userId || !db[userId]) {
        ...
        // মার্কেট মিটার লজিক
        const avgMom = Object.values(market).reduce((a,b)=>a+b.mom, 0) / CONFIG.coins.length;
        ...
        getBinanceBalance(user).then(balance => {
            ...
            <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
                ...
                <div class="p-6 bg-slate-900 rounded-[2.5rem] border border-sky-500/40 shadow-xl shadow-sky-500/10">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <div class="text-right">
                           <div class="text-[9px] font-bold text-slate-500 uppercase">Binance Balance</div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-6 bg-slate-900 rounded-[2rem] border border-slate-800 relative overflow-hidden shadow-2xl"><p class="text-[10px] text-slate-500 uppercase font-black">TOTAL PROFIT (BDT)</p><p class="text-4xl font-bold text-green-400 mt-2">৳${(user.profit * 124).toFixed(0)}</p><p class="text-xs text-slate-600 mt-1 italic">Life Time Gain</p><div class="card-icon">💲</div></div>
                ...
                <div class="bg-zinc-900/50 p-6 rounded-[2rem] border border-zinc-800 flex justify-between items-center shadow-lg">
                    <span class="text-xs font-bold uppercase text-slate-400 italic font-black">Trade Status</span>
                    <button onclick="location.href='/toggle-trade?id=${userId}'" class="px-6 py-2 rounded-full font-black text-[10px] uppercase transition ${user.isPaused ? 'bg-red-500/20 text-red-500 border border-red-500' : 'bg-green-500/20 text-green-400 border border-green-500'}">${user.isPaused ? 'PAUSED' : 'RUNNING'}</button>
                </div>
                ...
                <div class="p-6 bg-zinc-900/50 rounded-[2.5rem] border border-zinc-800 space-y-3 shadow-inner">
                    ${slots.map((s,i) => {
                        ...
                        return `<div class="p-4 bg-black/40 rounded-2xl border border-zinc-800/50 flex justify-between items-center"><div><span class="text-[9px] font-bold text-slate-600 italic uppercase font-black">Slot ${i+1}</span><p class="text-sm font-black ${s.active ? 'text-sky-400' : 'text-zinc-800'}">${s.active ? s.sym.replace('USDT','') : 'IDLE'}</p></div><div class="text-right">${s.active ? `<span class="text-xs font-bold ${s.pnl>=0?'text-green-500':'text-red-400'}">${s.pnl.toFixed(2)}% PNL</span>` : '<span class="text-[9px] text-zinc-700 font-black tracking-widest uppercase text-xs">Scanning</span>'}</div></div>${s.active && s.status === 'BOUGHT' ? `<div class="progress-bar"><div class="progress-fill" style="width: ${progress}%"></div></div>` : ''}`;
                    }).join('')}
                </div>

                <div class="text-center opacity-30"><button onclick="if(confirm('রিসেট করবেন?')) location.href='/reset-now?id=${userId}'" class="text-[9px] text-red-500 font-bold uppercase underline underline-offset-4 tracking-widest">Reset Master Core</button></div>
