...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            let feeR = u.fMode === 'bnb' ? 0.00018 : 0.0002; // লিমিট অর্ডারে মেকার ফি কম

            let aiSlots = walletBal < 30 ? 2 : (walletBal < 150 ? 4 : 6);
            if (u.userSlots.length !== aiSlots) {
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => u.userSlots[i] || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, pnl: 0, dca: 0, totalCost: 0, sellOrderId: null, entryTime: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'BUY_PENDING') {
                    if (ms.p <= sl.buy || u.mode === 'demo') {
                        ...
                        // 🚀 SELL LIMIT = কেনা দাম + ০.৪০% লাভ + এক্সিট ফি
                        let targetPrice = (sl.buy * (1 + 0.004 + feeR)).toFixed(COINS.find(c => c.s === sl.sym).d);
                        let sellOrder = await placeOrder(sl.sym, "SELL", sl.qty, u, targetPrice);
                        if (sellOrder) {
                            sl.sellOrderId = sellOrder.orderId; sl.status = 'SELL_PENDING';
                            sendTG(`🎯 <b>BUY HIT: #${sl.sym}</b>\nSell Limit set at: ${targetPrice}`, u.cid);
                        }
                    } else if ((Date.now() - sl.entryTime) / 1000 > 40) {
                ...
                if (sl.status === 'SELL_PENDING') {
                    let targetPrice = sl.buy * (1 + 0.004 + feeR);
                    if (ms.p >= targetPrice || u.mode === 'demo') {
                        let netProfitUSD = (parseFloat(sl.qty) * targetPrice) - sl.totalCost;
                        u.profit = (u.profit || 0) + netProfitUSD;
                        if(u.mode === 'demo') u.cap = Number(u.cap) + (sl.totalCost/u.lev) + netProfitUSD;
                        sendTG(`✅ <b>PROFIT: #${sl.sym}</b>\nNet 0.40% booked.`, u.cid);
                        Object.assign(sl, { active: false, status: 'IDLE', sym: '', dca: 0 }); saveDB();
                    }
                    return;
                }
            });

            // 🚀 SNIPER LIMIT ENTRY (এন্ট্রি ফি বাদ দিয়ে লিমিট)
            ...
            if (!u.isPaused && activeCount < aiSlots) {
                for (let sym of Object.keys(market)) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === sym)) {
                        ...
                        // লিমিট প্রাইস = বর্তমান দাম - এন্ট্রি ফি
                        ...
                        if (sIdx !== -1) {
                            ...
                            if (order) {
                                ...
                                saveDB(); sendTG(`⏳ <b>LIMIT SET: #${sym}</b>\nEntry Price: ${limitPrice}`, u.cid);
                            }
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">Quantum AI Pulse</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v25</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; d.userSlots.forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${s.active ? statusColor : 'text-zinc-700'} tracking-wider">\${s.active ? s.sym + ' ['+s.status+']' : 'Slot '+(i+1)+' Idle'}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Limit: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Fee: Maker</div><div>Target: \${(s.buy * 1.004).toFixed(4)}</div><div class="text-right italic">Quantum v25</div></div>\` : ''}</div>\`;
