...
// ==============================================
// 🛡️ Quantum AI Master v78.0 - Alpha Edition
// ==============================================
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                let dcaT = sl.dca === 0 ? -2.2 : -5.5; // Small cap risk management
                if (rawPnL <= dcaT && sl.dca < (u.cap < 10 ? 3 : 5) && (sl.totalCost/u.lev)*2 < u.cap*0.9) {
            ...
            let inSafety = u.userSlots.some(s => s.active && s.dca >= 3);
            if (!u.isPaused && sIdx !== -1 && !inSafety) {
                let rLim = u.tSpeed === 'fast' ? 55 : (u.tSpeed === 'safe' ? 35 : 45);
                let dLim = u.tSpeed === 'fast' ? 0.9975 : (u.tSpeed === 'safe' ? 0.9930 : 0.9960);
                for (let sym of Object.keys(market)) {
                    const ms = market[sym]; if (ms.p === 0 || (u.cooldowns[sym] && Date.now() < u.cooldowns[sym])) continue;
                    if (ms.p < (Math.max(...ms.history) * dLim) && calculateRSI(ms.history) < rLim && ms.p > (ms.low * 1.0002)) {
