...
// ==============================================
// 🛡️ Quantum AI v1000.0 - THE BOTTOM HUNTER
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'nebula_master_hub_v1000.json';
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            let currentMarginTotal = u.userSlots.reduce((a, s) => a + (s.active ? s.totalCost/u.lev : 0), 0);
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // ইন্টেলিজেন্ট আর্লি DCA (সাধ্যমতো)
            ...
            // অবিরাম হান্টিং লজিক (টাকা থাকলে স্লট ফাঁকা রাখবে না)
            let currentTotalProfitBDT = Number(u.profit || 0) * 124;
            if (currentTotalProfitBDT < Number(u.targetBDT) && !u.isPaused && activeTrades < u.slots) {
                ...
                for (let sym of Object.keys(market)) {
                    ...
                    // এন্ট্রি লজিক: তলা ডিটেকশন (RSI 30-35) + রিভার্সাল (Green Tick)
                    if (m.rsi < rLim && m.p < (Math.max(...m.history) * dLim) && m.p > (m.low * 1.0005)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (sIdx !== -1 && await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                activeTrades++; saveDB(); sendTG(`🚀 <b>BOTTOM ENTRY: #${sym}</b>\nPrice: $${m.p.toFixed(4)}\nMargin: $${mE.toFixed(4)}`, u.cid);
                            }
                        }
                    }
                }
