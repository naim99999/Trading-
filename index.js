...
// ==============================================
// 🛡️ Quantum AI Master v80.0 - Apex Predator
// ==============================================
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (u.isAuto) {
                ...
                u.tSpeed = btcT > 0.04 ? "fast" : (btcT < -0.12 ? "safe" : "normal");
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (rawPnL <= dcaT && sl.dca < (u.cap < 10 ? 2 : 4) && (sl.totalCost/u.lev)*2 < u.cap*0.9) {
                    if (await placeOrder(sl.sym, "BUY", sl.qty, u)) {
                        ...
                        sl.buy = sl.totalCost / parseFloat(sl.qty); sl.dca++; sl.sell = sl.buy * 1.0035; sl.be = false; saveDB();
                        sendTG(`🌀 <b>DCA: #${sl.sym} (L${sl.dca})</b>\n💰 খরচ: $${stM.toFixed(4)}\n📉 মোট: $${(sl.totalCost/u.lev).toFixed(2)}`, u.cid);
                    }
                }
            ...
            if (!u.isPaused && sIdx !== -1 && !anyDanger) {
                ...
                let dLim = u.tSpeed === 'fast' ? 0.9985 : (u.tSpeed === 'safe' ? 0.9940 : 0.9970);
