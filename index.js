...
// ==============================================
// 🛡️ Quantum AI Master v1000.3 - AI & TARGET PRO
// ==============================================
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            // --- AI Pulse Logic (অটোমেটিক পুস কন্ট্রোল) ---
            if (btcT < -0.12) { u.aiPaused = true; } 
            else if (btcT > -0.04) { u.aiPaused = false; }

            // যদি টার্গেট ফিল হয়ে যায় এবং সব স্লট খালি হয়ে যায়, তবে বট কমপ্লিট করে দিন
            if (targetReached && activeTrades === 0) {
                u.status = 'COMPLETED'; saveDB();
                sendTG(`🎯 <b>TARGET REACHED!</b>\nTotal Profit: ৳${currentProfitBDT.toFixed(2)}\nBot has stopped safely.`, u.cid);
                continue;
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // --- স্মার্ট এক্সিট লজিক ---
                let minP = 1.00; // নরমাল প্রফিট ১ টাকা
                if (u.isPaused || u.aiPaused) minP = 0.20; // পুস অবস্থায় দ্রুত বের হবে (২০ পয়সা)
                if (targetReached) minP = 0.01; // টার্গেট ফিল হলে লাভ পেলেই বের হবে (১ পয়সা)

                let dropTrigger = sl.maxNetBDT - (targetReached ? 0.001 : 0.01);

                if (sl.netBDT >= minP && (u.isPaused || u.aiPaused || targetReached || (sl.maxNetBDT > 0 && sl.netBDT <= dropTrigger))) {
            ...
            // --- হান্টিং লজিক (AI পুস বা ম্যানুয়াল পুস থাকলে এন্ট্রি হবে না) ---
            if (!u.isPaused && !u.aiPaused && !targetReached && activeTrades < u.slots) {
                if (btcT < -0.07) return; 
                for (let sym of Object.keys(market)) {
                    if (activeTrades >= u.slots) break;
                    const m = market[sym]; if (m.p === 0 || m.history.length < 20) continue;
                    if (m.rsi < 50 && m.p < (Math.max(...m.history) * 0.9985)) {
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">BTC AI PULSE</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div id="aiStat" class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ AI ACTIVE</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                document.getElementById('pauseBtn').innerText = d.isPaused ? "RESUME" : "MANUAL PAUSE";
                
                const aiStat = document.getElementById('aiStat');
                if(d.aiPaused) { aiStat.innerText = "⚠️ AI: SAFE MODE"; aiStat.className = "px-3 py-2 bg-red-600/20 border border-red-500/50 rounded-lg text-[8px] font-black text-red-400"; }
                else { aiStat.innerText = "🛡️ AI: ACTIVE"; aiStat.className = "px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400"; }
                if((d.profit*124) >= d.targetBDT) { aiStat.innerText = "🎯 TARGET COMPLETED"; aiStat.className = "px-3 py-2 bg-green-600/20 border border-green-500/50 rounded-lg text-[8px] font-black text-green-400"; }
