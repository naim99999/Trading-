...
// ==============================================
// 👑 QUANTUM AI MASTER v86.0 - VOLUME EXHAUSTION GUARD
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v86_master.json';
...
let market = {};
COINS.forEach(c => market[c.s] = { p: 0, b: 0, history: [], ema7: 0, rsi: 50, btcTrend: 0, buyVol: 0, sellVol: 0, lastVol: 0 });
...
async function startGlobalEngine() {
    ...
    ws.on('message', (data) => {
        ...
        if (msg.stream === "!ticker@arr") {
            ...
            const d = msg.data; if (market[d.s]) { const s = market[d.s]; s.b = parseFloat(d.b); s.lastVol = s.buyVol; s.buyVol = parseFloat(d.B); s.sellVol = parseFloat(d.A); }
        }
    });
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            let aiSlots = walletBal < 30 ? 2 : (walletBal < 200 ? 4 : 5);
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                let existing = (u.userSlots || []).filter(s => s.active);
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => existing.find(x => x.id === i) || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, curP: 0, dca: 0, maxP: 0, totalFeePaid: 0, netBDT: 0, pnl: 0, lastDcaTime: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                let netUSD = (tradeVal - sl.totalCost) - (sl.totalFeePaid + exitFee);
                ...
                if (sl.status === 'ACTIVE') {
                    if (parseFloat(sl.netBDT) >= 0.50) {
                        ...
                        // 📊 পয়েন্ট ২: ভলিউম এবং বায়ার প্রেশার এনালাইসিস
                        let buyerPressure = ms.buyVol / (ms.buyVol + ms.sellVol);
                        let volDropped = ms.buyVol < (ms.lastVol * 0.8); // বায়ার ভলিউম ২০% কমলে
                        let dropFromPeak = ((sl.maxP - ms.p) / sl.maxP) * 100;

                        // এক্সিট কন্ডিশন: দাম শিখর থেকে নামলে OR বায়ার প্রেশার কমে সেলার বাড়লে
                        if (dropFromPeak > 0.03 || buyerPressure < 0.45 || volDropped) {
                            sl.isClosing = true;
                            if (await placeOrder(sl.sym, "SELL", sl.qty, u)) {
                                ...
                                sendTG(`✅ <b>VOLUME EXIT! #${sl.sym}</b>\nGain: ৳${sl.netBDT}\nPressure: ${(buyerPressure*100).toFixed(0)}%`, u.cid);
                    ...
                    else if (parseFloat(sl.pnl) < -4.0 && sl.dca < 5 && (Date.now() - sl.lastDcaTime) > 300000 && walletBal > (sl.marginUsed * 1.5)) {
                        if (ms.p > ms.ema7) {
                            ...
                            if (await placeOrder(sl.sym, "BUY", addQty, u)) {
                                ...
                                sl.totalFeePaid += (parseFloat(addQty) * ms.p * feeR);
            ...
            if (!u.isPaused && !isTargetDone && activeCount < (aiSlots - 1)) {
                for (let coin of COINS) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && buyerPressure > 0.60 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        let entryVal = (walletBal * u.lev) / aiSlots / 5;
                        ...
                        if (sIdx !== -1 && walletBal > 10) {
                            if (await placeOrder(coin.s, "BUY", qty, u)) {
                                ...
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'ACTIVE', sym: coin.s, buy: m.p, qty: qty, totalCost: (parseFloat(qty) * m.p), marginUsed: mUsed, totalFeePaid: (parseFloat(qty) * m.p * feeR), curP: m.p, dca: 0, pnl: 0, maxP: m.p, isClosing: false, lastDcaTime: 0, netBDT: 0 };
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v86 PRO</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=\${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '%' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Gain: ৳\${s.netBDT}</div><div class="text-green-400 font-black">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum v86</div></div>\` : ''}</div>\`;
