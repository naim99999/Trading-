...
// ==============================================
// 👑 QUANTUM master engine v62.0 - DCA FEE SYNC
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v62_final.json';
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // 🧮 নিখুঁত নিট লাভ ক্যালকুলেশন (DCA ফি সহ)
                ...
                if (sl.status === 'SELL_PENDING') {
                    ...
                    let reversing = (sl.maxNetBDT - parseFloat(sl.netBDT)) > 0.06;
                    ...
                    if ((ms.p >= sl.targetP || reversing) && parseFloat(sl.netBDT) >= 0.50) {
                        ...
                        sendTG(`✅ <b>PROFIT! #${sl.sym}</b>\n━━━━━━━━━━━━━━\n💰 Gain: <b>৳${sl.netBDT}</b>\n📉 Fees: <b>$${(sl.totalFeePaid + exitFee).toFixed(3)}</b>\n🏦 Wallet: <b>$${Number(u.cap || walletBal).toFixed(2)}</b>`, u.cid);
                        ...
                        if (ms.p > ms.ema7) {
                            let addQty = (parseFloat(sl.qty) * 1.35).toFixed(COINS.find(c => c.s === sl.sym).qd);
                            let addCost = parseFloat(addQty) * ms.p;
                            let entryFee = addCost * feeR; // DCA ফি ক্যালকুলেশন
                            let addM = addCost / u.lev;

                            if (u.mode === 'demo') u.cap = Number(u.cap || 0) - addM;
                            sl.totalCost += addCost;
                            sl.totalFeePaid += entryFee; // মোট ফি-তে যোগ হলো
                            ...
                            saveDB();
                            sendTG(`🌀 <b>DCA APPLIED: #${sl.sym}</b>\nAvg Price: ${sl.buy.toFixed(4)}\nNew Fee Logic Applied.`, u.cid);
                        }
                    }
                }
            });
            ...
            if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < (aiSlots - 1)) {
                for (let coin of COINS) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        ...
                        let firstFee = (parseFloat(qty) * parseFloat(limitPrice)) * feeR;

                        const sIdx = u.userSlots.findIndex(sl => !sl.active);
                        if (sIdx !== -1 && walletBal > (mNeeded + 5)) {
                            if (await placeOrder(coin.s, "BUY", qty, u, limitPrice)) {
                                if(u.mode === 'demo') u.cap = Number(u.cap || 0) - mNeeded;
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: coin.s, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: mNeeded, totalFeePaid: firstFee, entryTime: Date.now(), curP: m.p, dca: 0, pnl: 0, maxNetBDT: 0 };
                                saveDB();
                            }
                        }
                    }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v62 PRO</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '% (৳'+s.netBDT+')' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Goal: \${(s.targetP || 0).toFixed(4)}</div><div class="text-green-400">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum v62</div></div>\` : ''}</div>\`;
