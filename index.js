...
// ==============================================
// 👑 QUANTUM AUTO-PILOT v53.0 - THE FINAL ENGINE
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_autopilot_v53.json';
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            // 🎯 স্লট অটো সেটআপ (ক্যাপিটাল অনুযায়ী)
            ...
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                ...
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => existing.find(x => x.id === i) || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, dca: 0, maxNetBDT: 0, entryTime: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'SELL_PENDING') {
                    let totalValue = parseFloat(sl.qty) * ms.p;
                    let estFees = (sl.totalCost + totalValue) * feeR;
                    let netUSD = (totalValue - sl.totalCost) - estFees;
                    let netBDT = netUSD * 124;

                    // 🎯 PROFIT HUNTER: নিট ০.৫০ টাকা এবং টার্গেট প্রাইস টাচ করলে
                    if (ms.p >= sl.targetP && netBDT >= 0.50) {
                        if (!sl.maxNetBDT || netBDT > sl.maxNetBDT) sl.maxNetBDT = netBDT;
                        // ট্রেইলিং: লাভ যদি সর্বোচ্চ শিখর থেকে সামান্য ড্রপ করে
                        if ((sl.maxNetBDT - netBDT) > 0.05 || netBDT > 100) {
                    ...
                    // 🌀 SMART RECOVERY DCA (ক্যাপিটাল চেক করে)
                        ...
                        if (ms.p > ms.ema7) { // রিভার্সাল কনফার্মেশন
                            ...
                            saveDB(); sendTG(`🌀 <b>AI RECOVERY: #${sl.sym}</b>\nNew Avg Price: ${sl.buy.toFixed(4)}`, u.cid);
                        }
                    }
                    return;
                }
            ...
            if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < (aiSlots - 1)) {
                for (let coin of COINS) {
                    ...
                    if (walletBal < 30 && coin.r) continue; // ছোট ব্যালেন্সে রিস্কি কয়েন বন্ধ
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '%' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Goal: \${(s.targetP || 0).toFixed(4)}</div><div class="text-green-400 font-black">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum v53</div></div>\` : ''}</div>\`;
