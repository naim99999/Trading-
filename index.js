...
// ==========================================
// 🛡️ Quantum AI - Precision Bottom Finder v60.0
// ==========================================
...
const COINS = [
    ...
    { s: "GALAUSDT", n: "GALA", d: 5, qd: 0 }, { s: "RUNEUSDT", n: "RUNE", d: 3, qd: 1 }
];
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            userSlots[userId].forEach(async (sl) => {
                ...
                // উন্নত ডিএসিএ গ্যাপ (১ম বার ২.৫% ড্রপ প্রয়োজন)
                let dcaTrigger = sl.dca === 0 ? -2.5 : (sl.dca === 1 ? -5.0 : -9.5);
                if (rawPnL <= dcaTrigger && sl.dca < 3) {
                    ...
                    if (order) {
                        ...
                        sendTG(`🌀 <b>Recovery:</b> #${sl.sym} (Level ${sl.dca}) এভারেজ করা হয়েছে।`, config.cid);
                    }
                }
            ...
            // ৫. স্মার্ট প্রিসিশন এন্ট্রি লজিক
            const slotIdx = userSlots[userId].findIndex(sl => !sl.active);
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 3) { // ৩ বার দাম বাড়তে হবে (ধৈর্য্য)
                const localHigh = Math.max(...s.history);
                const isPullback = s.p < localHigh * 0.9985; // চুড়া থেকে ০.১৫% ড্রপ
                
                if (isPullback && s.p < sma) { // ডিপে আছে কি না চেক
                    ...
                    if (sameCoin.length === 0) {
                        ...
                        if (order) {
                            ...
                            sendTG(`🚀 <b>PRECISION ENTRY:</b> #${msg.s} (Bottom Found)`, config.cid);
                        }
                    }
                }
            }
...
// ... Server & UI Logic (Same as v52.0 for stability)
