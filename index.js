...
const COINS = [
    ...
    { s: "GALAUSDT", d: 5, qd: 0, r: true }, { s: "PEPEUSDT", d: 8, qd: 0, r: true }
];

let market = {};
COINS.forEach(c => market[c.s] = { p: 0, history: [], ema7: 0, rsi: 50, btcTrend: 0, vol: 0 });

function calculateEMA(p, n) { if (p.length < n) return p[p.length-1] || 0; let k = 2/(n+1), ema = p[0]; for(let i=1; i<p.length; i++) ema = p[i]*k + ema*(1-k); return ema; }
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            // 🛡️ AI PANIC GUARD: BTC ক্রাশ করলে নতুন এন্ট্রি এবং DCA সাময়িক বন্ধ
            let isPanic = market["BTCUSDT"]?.btcTrend < -0.4;
            ...
            if (!u.userSlots || u.userSlots.length !== aiSlotsCount) {
                ...
                u.userSlots = Array(aiSlotsCount).fill(null).map((_, i) => existing.find(x => x.id === i) || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, curP: 0, dca: 0, entryTime: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'BUY_PENDING') {
                    let waitSec = (Date.now() - sl.entryTime) / 1000;
                    if (ms.p <= sl.buy) {
                        sl.status = 'ACTIVE'; sl.entryTime = Date.now();
                        let tVal = (sl.id % 2 === 0) ? Number(u.evenT) : Number(u.oddT);
                        // 🤖 AI VOLATILITY ADAPTATION: যদি RSI খুব বেশি হয়, তবে বড় টার্গেট খুঁজবে
                        let adjustedTarget = ms.rsi > 70 ? tVal * 1.5 : tVal;
                        let targetPrice = (sl.buy * (1 + (adjustedTarget/100) + feeR)).toFixed(COINS.find(c => c.s === sl.sym).d);
                        sl.targetP = parseFloat(targetPrice);
                        if (await placeOrder(sl.sym, "SELL", sl.qty, u, targetPrice)) {
                            sl.status = 'SELL_PENDING'; saveDB();
                            sendTG(`🚀 <b>BUY HIT: #${sl.sym}</b>\nAI Target set: ${targetPrice} (${adjustedTarget.toFixed(2)}%)`, u.cid);
                        }
                    } else if (waitSec > 60) {
                        await cancelOrder(sl.sym, sl.orderId, u);
                ...
                if (sl.status === 'SELL_PENDING') {
                    ...
                    // 🌀 AI SMART RECOVERY (BTC প্যানিক না থাকলে এবং বুলিশ রিভার্সাল হলে)
                        ...
                        if (ms.p > ms.ema7) { 
                            ...
                            saveDB(); sendTG(`🌀 <b>AI DCA: #${sl.sym}</b>\nAvg Optimized at Reversal Point.`, u.cid);
                        }
                    }
                    return;
                }
            ...
            if (!u.isPaused && !isTargetDone && !isPanic && activeCount < (aiSlotsCount - 1)) {
                for (let coin of COINS) {
                    const m = market[coin.s]; if (m.p === 0 || m.history.length < 50) continue;
                    if (walletBal < 35 && coin.r) continue;

                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        let entryVal = (walletBal * u.lev) / aiSlotsCount / 4.2;
                        ...
                        if (sIdx !== -1 && walletBal > (mNeeded + 5)) {
                            let buyOrder = await placeOrder(coin.s, "BUY", qty, u, limitPrice);
                            if (buyOrder) {
                                if(u.mode === 'demo') u.cap = parseFloat((Number(u.cap) - mNeeded).toFixed(2));
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: coin.s, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: mNeeded, entryTime: Date.now(), curP: m.p, dca: 0, orderId: buyOrder.orderId };
                                saveDB();
                            }
                        }
                    }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/register') { 
        ...
        cachedUsers[q.get('id')] = { api: q.get('api'), sec: q.get('sec'), cid: q.get('cid'), cap: Number(q.get('cap')), targetBDT: Number(q.get('target')), lev: Number(q.get('lev')), evenT: q.get('evenT'), oddT: q.get('oddT'), mode: q.get('mode'), fMode: q.get('fmode'), profit: 0, isPaused: false, userSlots: [] };
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">AI Status</p><p class="text-[10px] font-black" id="apiS">Checking...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v45 AI</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                if(d.apiStatus === "CONNECTED") { apiS.innerText = "🟢 Connected"; apiS.className="text-[10px] font-black text-green-400"; }
                else if(d.apiStatus === "DEMO") { apiS.innerText = "🔵 Demo Mode"; apiS.className="text-[10px] font-black text-sky-400"; }
                else { apiS.innerText = "🔴 API Error"; apiS.className="text-[10px] font-black text-red-500"; }
                const pM = document.getElementById('apiS'); document.getElementById('pP').innerText = "BTC: $" + (d.btcPrice || "0.00");
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '%' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Goal: \${(s.targetP || 0).toFixed(4)}</div><div class="text-green-400">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum PRO</div></div>\` : ''}</div>\`;
