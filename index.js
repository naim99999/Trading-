...
// ==============================================
// 👑 QUANTUM AI MASTER v88.0 - THE FINAL STABLE
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v88_final.json';
...
let market = {};
COINS.forEach(c => market[c.s] = { p: 0, history: [], ema7: 0, ema25: 0, rsi: 50, btcTrend: 0, buyVol: 0, sellVol: 0, lastV: 0 });
...
async function startGlobalEngine() {
    ...
    ws.on('message', (data) => {
        ...
        raw.forEach(d => { if (market[d.s]) { const s = market[d.s]; s.p = parseFloat(d.c); s.history.push(s.p); if(s.history.length > 60) s.history.shift(); s.ema7 = calculateEMA(s.history, 7); s.ema25 = calculateEMA(s.history, 25); s.rsi = calculateRSI(s.history); s.lastV = s.buyVol; s.buyVol = parseFloat(d.q); if (d.s === "BTCUSDT" && s.history.length > 10) s.btcTrend = ((s.p - s.history[0]) / s.history[0] * 100); } });
    });
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            let aiSlots = walletBal < 35 ? 1 : (walletBal < 200 ? 3 : 5);
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => ({ id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, dca: 0, maxP: 0, totalFeePaid: 0, lastDcaTime: 0 }));
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'ACTIVE') {
                    if (parseFloat(sl.netBDT) >= 0.50) {
                        ...
                        let dropLimit = ((sl.maxP - ms.p) / sl.maxP) * 100;
                        let volumeExhausted = ms.buyVol < (ms.lastV * 0.7);

                        if (dropLimit > 0.04 || volumeExhausted || sl.pnl > 8.0) {
                            sl.isClosing = true;
                        ...
                        if (ms.p > ms.ema7) {
                            ...
                            if (await placeOrder(sl.sym, "BUY", addQty, u)) {
                                ...
                                sendTG(`🌀 <b>AI RECOVERY: #${sl.sym}</b>`, u.cid);
                            }
                        }
                    }
                }
            ...
            if (!u.isPaused && !isTargetDone && activeCount < aiSlots && market["BTCUSDT"]?.btcTrend > -0.5) {
                for (let coin of COINS) {
                    const m = market[coin.s]; if (m.p === 0 || m.history.length < 50) continue;
                    let buyerPressure = m.buyVol / (m.buyVol + m.sellVol);
                    if (m.rsi < 28 && m.p > m.ema7 && m.ema7 > m.ema25 && buyerPressure > 0.60 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900 rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI STATUS METER</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v88 PRO</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=\${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '%' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Gain: ৳\${s.netBDT}</div><div class="text-green-400 font-black">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum v88</div></div>\` : ''}</div>\`;
