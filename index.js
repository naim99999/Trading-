...
// ==========================================
// 🛡️ Quantum AI - Speed-Hunter Core v63.0
// ==========================================
...
const COINS = [
    ...
    { s: "ARBUSDT", n: "ARB", d: 4, qd: 1 }, { s: "GALAUSDT", n: "GALA", d: 5, qd: 0 },
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            userSlots[userId].forEach(async (sl) => {
                ...
                // ১. ডায়নামিক ট্রেলিং শিল্ড ও প্রফিট
                if (rawPnL >= 0.35) {
                ...
                // ২. অসীম DCA রিকভারি (১.৫%, ৩.৫%, ৭.০%)
                ...
                // ৩. ইন্টেলিজেন্ট ক্লোজিং (Trend > 7 ওয়েটিং সহ)
                if (s.p >= sl.sell || (sl.be && s.p <= sl.slP)) {
                    if (s.p >= sl.sell && s.trend > 7) return; // রকেটের মতো উঠলে সেল করো না
                    ...
                    if ((netGain * 124) >= 1) {
                        ...
                        const report = `✅ <b>TRADE CLOSED!</b>\n🔸 #${sl.sym}\n💵 লাভ: ৳${(netGain * 124).toFixed(0)}\n📈 মোট সাগর: ৳${(config.profit * 124).toFixed(0)}`;
                        sendTG(report, config.cid);
                        if(config.mode !== 'demo') await placeOrder(sl.sym, "SELL", sl.qty, config);
                        setTimeout(() => { sl.active = false; sl.status = 'IDLE'; }, 1000); // দ্রুত রিস্টার্ট
                    }
                }
            });

            // ৪. হাইপার পুলব্যাক এন্ট্রি (লিমিটের জন্য বসে থাকবে না)
            ...
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 1) {
                ...
                const isPullback = s.p < localHigh * 0.9990; // সর্বোচ্চ দাম থেকে মাত্র ০.১০% ড্রপ

                if (isPullback && s.p < sma) {
                    ...
                    if (sameCoin.length === 0) {
                        ...
                        if (order) {
                            ...
                            sendTG(`🚀 <b>FAST ENTRY:</b> #${msg.s} (Dip Hit)`, config.cid);
                        }
                    }
                }
            }
...
// UI & API (v51.0 লজিক অক্ষুণ্ণ রাখা হয়েছে)
