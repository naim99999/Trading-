            ...
            slots.forEach(async (sl) => {
                ...
                if (sl.status === 'BOUGHT') {
                    ...
                    if (s.p >= sl.sell) {
                        ...
                        sendTG(`🎉 *SOLD SUCCESS!* ${sl.sym} \nGain: ৳${(gain*124).toFixed(0)}`, config.cid, sl.mid);
                    ...
                    ${slots.map((s,i) => {
                        ...
                        return `<div class="p-4 bg-black/40 rounded-2xl border border-zinc-800/50">
                            <div class="flex justify-between items-center mb-1">
                                ...
                                <div class="text-right">${s.active ? `<span class="text-xs font-bold ${s.pnl>=0?'text-green-500':'text-red-400'}">${(s.pnl||0).toFixed(2)}% PNL</span>` : '<span class="text-[9px] text-zinc-700 font-black tracking-widest uppercase text-xs animate-pulse">Scanning</span>'}</div>
                            </div>
