...
const DB_FILE = 'nebula_master_final_sync.json';
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            slots.forEach(async (sl) => {
                ...
                // বাই হওয়ার সাথে সাথে ৩টি অর্ডার বিন্যান্সে পাঠানো (Vision Update)
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen text-center"><div class="max-w-md mx-auto space-y-6 w-full">
            ...
            <form action="/register" method="GET" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 text-left shadow-2xl border border-sky-500/10">
                ...
                <input name="api" placeholder="API Key" class="w-full bg-black p-4 rounded-2xl border border-slate-800 text-white outline-none"><input name="sec" placeholder="Secret Key" class="w-full bg-black p-4 rounded-2xl border border-slate-800 text-white outline-none">
                    ...
                    ${slots.map((s,i) => {
                        ...
                        return `<div class="p-4 bg-black/40 rounded-2xl border border-zinc-800/50">
                            <div class="flex justify-between items-center mb-1">
                                ...
                                <div class="text-right">${s.active ? `<span class="text-xs font-bold ${s.pnl>=0?'text-green-500':'text-red-400'}">${s.pnl.toFixed(2)}% PNL</span>` : '<span class="text-[9px] text-zinc-700 font-black tracking-widest uppercase text-xs animate-pulse">Scanning</span>'}</div>
                            </div>
