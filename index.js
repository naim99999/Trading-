...
// ==========================================
// 🛡️ Quantum AI - Master Intelligent DCA v40.0
// ==========================================
...
const COINS = [
    ...
    { s: "PEPEUSDT", n: "PEPE", d: 7, qd: 0 }, { s: "GALAUSDT", n: "GALA", d: 5, qd: 0 }
];
...
async function sendTG(msg, chatId) {
    try { await axios.post(`https://api.telegram.org/bot${MASTER_TG_TOKEN}/sendMessage`, { chat_id: chatId || FIXED_CHAT_ID, text: msg, parse_mode: 'Markdown' }); } catch (e) {}
}
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            userSlots[userId].forEach(async (sl) => {
                ...
                // ১. ডায়নামিক প্রফিট ট্রেলিং
                let currentTargetPct = ((sl.sell - sl.buy) / sl.buy) * 100 * config.lev;
                if (rawPnL >= currentTargetPct * 0.9) {
                    sl.sell = sl.buy * (1 + (currentTargetPct + 0.1) / (100 * config.lev));
                    sl.slP = sl.buy * (1 + (rawPnL - 0.1) / (100 * config.lev));
                    sl.be = true;
                }
                ...
                if (sl.dca < 3 && rawPnL <= -1.8) {
                    ...
                    if (order) {
                        ...
                        sl.dca += 1; sl.sell = sl.buy * 1.0040; sl.be = false;
                        sendTG(`🌀 *DCA Executed:* #${sl.sym} গড় ক্রয়মূল্য কমানো হয়েছে।`, config.cid);
                    }
                }

                // ৪. প্রফিট ক্লোজ ও বিস্তারিত রিপোর্ট
                if (s.p >= sl.sell) {
                    if (s.trend > 7) return; 

                    const gain = (sl.qty * s.p) - (sl.totalCost);
                    const netGain = gain * 0.9985;
                    sl.active = false; config.profit += netGain; config.count += 1;
                    saveUser(userId, config);
                    
                    const report = `✅ *TRADE PROFIT LOCKED!*\n\n` +
                                 `🔸 *Coin:* #${sl.sym}\n` +
                ...
                if (sl.be && s.p <= sl.slP) {
                    const gain = (sl.qty * s.p) - (sl.totalCost);
                    const netGain = gain * 0.9985;
                    sl.active = false; config.profit += Math.max(0, netGain);
                    saveUser(userId, config);
                    sendTG(`🛡️ *Shield Hit:* #${sl.sym} সামান্য লাভে ক্লোজ।\n💰 লাভ: ৳${(netGain*124).toFixed(0)}`, config.cid);
                    if(config.mode !== 'demo') await placeOrder(sl.sym, "SELL", sl.qty, config);
                }
            });

            // ৬. এন্ট্রি লজিক (সুপার ফাস্ট)
            ...
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 1) {
                ...
                if (sameCoin.length === 0) {
                    ...
                    if (order) {
                        userSlots[userId][slotIdx] = { id: slotIdx, active: true, sym: msg.s, buy: buyPrice, sell: buyPrice * 1.0040, slP: 0, qty: qty, pnl: 0, curP: s.p, dca: 0, totalCost: (qty * buyPrice), be: false };
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 flex items-center min-h-screen text-center font-black uppercase italic tracking-tighter"><div class="max-w-md mx-auto w-full space-y-6">
            ...
            <form action="/register" method="GET" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 border border-slate-800 text-left tracking-normal not-italic font-sans">
            ...
            <body class="bg-[#020617] text-white p-4 font-sans uppercase">
                <div class="max-w-xl mx-auto space-y-4">
                    <div class="p-6 bg-slate-900 rounded-[2.5rem] border-2 border-sky-500/50 text-center shadow-2xl">
                        <p class="text-[10px] text-sky-400 font-bold mb-1 tracking-widest uppercase">Available Binance Balance</p>
                        <p class="text-5xl font-black text-white">$<span id="balanceText">0.00</span></p>
                    </div>
                    <div class="grid grid-cols-2 gap-4 text-center italic">
                        <div class="p-6 bg-slate-900 rounded-[2.5rem] border border-slate-800"><p class="text-[9px] text-slate-500 font-bold mb-1 uppercase tracking-widest">Growth (BDT)</p><p class="text-4xl font-black text-green-400">৳<span id="profitText">0</span></p></div>
                        <div class="p-6 bg-slate-900 rounded-[2.5rem] border border-slate-800"><p class="text-[9px] text-slate-500 font-bold mb-1 uppercase tracking-widest">Completed</p><p class="text-4xl font-black text-sky-400" id="countText">0</p></div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 pt-4">
                        ...
                        <a href="/reset?id=${userId}" onclick="return confirm('রিসেট করবেন?')" class="flex-1 bg-red-900/20 border border-red-500/30 text-red-500 py-5 rounded-full text-center text-[10px] font-black tracking-widest">Reset</a>
                    </div>
                    <button onclick="location.reload()" class="w-full bg-sky-600 py-5 rounded-full text-[10px] font-black tracking-widest italic">Refresh Engine</button>
                </div>
                <script>
                    async function togglePause() { await fetch('/toggle-pause?id=${userId}'); setTimeout(()=>location.reload(), 500); }
                    async function updateData() {
                        try {
                            ...
                            pBtn.className = data.isPaused ? "flex-1 bg-green-900/20 border border-green-500/30 text-green-400 py-5 rounded-full text-[10px] font-black tracking-widest" : "flex-1 bg-orange-900/20 border border-orange-500/30 text-orange-400 py-5 rounded-full text-[10px] font-black tracking-widest";
                            let html = '';
                            data.slots.forEach((s, i) => {
                                let meter = s.active ? Math.max(0, Math.min(100, ((s.curP - s.buy) / (s.sell - s.buy)) * 100)) : 0;
                                html += \`<div class="p-5 bg-slate-900/50 rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase">
                                    ...
                                    <div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1 uppercase"><div>Entry: \${s.buy.toFixed(4)}</div><div class="text-right">Live: \${s.curP}</div><div class="text-orange-400">Recovery Mode</div><div class="text-right text-green-500">Target: \${s.sell.toFixed(4)}</div></div>\` : ''}
                                </div>\`;
                            });
...
server.listen(PORT, '0.0.0.0', () => { startGlobalEngine(); sendTG("🚀 *Quantum Core v40.0 Online!* যাত্রা সফল হোক।", FIXED_CHAT_ID); });
