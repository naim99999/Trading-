...
// ==========================================
// 🛡️ Quantum AI - Master Core v26.0 (DCA Recovery)
// ==========================================
...
async function startGlobalEngine() {
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            if (!userSlots[userId]) userSlots[userId] = Array(5).fill(null).map((_, i) => ({ id: i, active: false, sym: '', buy: 0, sell: 0, qty: 0, pnl: 0, curP: 0, dca: 0, totalCost: 0 }));
            ...
            userSlots[userId].forEach(async (sl) => {
                ...
                // ১. স্মার্ট DCA লজিক (দাম ২% কমলে ব্যাকআপ কিনবে)
                if (sl.dca < 3 && rawPnL <= -2.0) {
                    ...
                    if (order) {
                        ...
                        sl.dca += 1;
                        sl.sell = sl.buy * 1.0040; // ফী বাদে লাভ নিশ্চিত
                        sendTG(`🌀 *DCA Executed:* #${sl.sym} এভারেজ করা হয়েছে।`, config.cid);
                    }
                }

                // ২. লাভে ক্লোজ (০.৪০% টার্গেট)
                if (s.p >= sl.sell) {
            ...
            // ৩. এন্ট্রি লজিক (৫ স্লট, পজ সাপোর্ট, ডাইনামিক ক্যাপিটাল ক্যালকুলেশন)
            ...
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 2) {
                ...
                if (sameCoin.length === 0) {
                    ...
                    // আপনার ক্যাপিটালের ১/১২ অংশ প্রথম এন্ট্রিতে নিবে (বাকিটা DCA ব্যাকআপ)
                    ...
                    if (order) {
                        ...
                        sendTG(`🚀 *NEW ENTRY:* #${msg.s}\nটার্গেট: ${sellPrice.toFixed(coin.d)}`, config.cid);
                    }
                }
            }
        }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/test-tg') {
        const uid = url.searchParams.get('id');
        const success = await sendTG("🔔 *Connection Check:* ড্যাশবোর্ড ও টেলিগ্রাম কানেক্টেড!", db[uid]?.cid);
            ...
            <body class="bg-[#020617] text-white p-4 font-sans uppercase">
                <div class="max-w-xl mx-auto space-y-4">
                    <div class="p-6 bg-slate-900 rounded-[2.5rem] border-2 border-sky-500/50 text-center shadow-2xl">
                        <p class="text-[10px] text-sky-400 font-bold mb-1 tracking-widest">Available Balance</p>
                        <p class="text-5xl font-black text-white">$<span id="balanceText">0.00</span></p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-5 bg-slate-900 rounded-[2.5rem] text-center border border-slate-800 shadow-xl"><p class="text-[9px] text-slate-500 font-bold mb-1 tracking-widest uppercase">Growth (BDT)</p><p class="text-4xl font-black text-green-400">৳<span id="profitText">0</span></p></div>
                        <div class="p-5 bg-slate-900 rounded-[2.5rem] text-center border border-slate-800 shadow-xl"><p class="text-[9px] text-slate-500 font-bold mb-1 tracking-widest uppercase">Wins</p><p class="text-4xl font-black text-sky-400" id="countText">0</p></div>
                    </div>
                    <div id="slotContainer" class="space-y-3"></div>
                    <div class="space-y-3 pt-4">
                        <button onclick="checkTG()" id="tgBtn" class="w-full bg-sky-900/30 border border-sky-500/30 text-sky-400 py-4 rounded-full text-[10px] font-black tracking-widest">CHECK TELEGRAM CONNECTION</button>
                        <div class="grid grid-cols-2 gap-3">
                            <button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-500">PAUSE SYSTEM</button>
                            <a href="/reset?id=${userId}" onclick="return confirm('রিসেট করবেন?')" class="bg-red-900/20 border border-red-500/30 text-red-500 py-5 rounded-full text-center text-[10px] font-black uppercase">Reset Data</a>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                ...
                <script>
                    ...
                    async function updateData() {
                        try {
                            ...
                            if(data.isPaused) { pBtn.innerText = "RESUME SYSTEM"; pBtn.className = "flex-1 bg-green-900/20 border border-green-500/30 text-green-400 py-5 rounded-full text-[10px] font-black"; }
                            ...
                            data.slots.forEach((s, i) => {
                                ...
                                html += \`<div class="p-5 bg-slate-900/50 rounded-3xl border border-zinc-800 transition-all duration-300">
                                    ...
                                    <div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Entry: \${s.buy.toFixed(4)}</div><div class="text-right">Live: \${s.curP}</div><div class="text-orange-400">DCA: \${(s.buy * 0.985).toFixed(4)}</div><div class="text-right text-green-500">Target: \${s.sell.toFixed(4)}</div></div>\` : ''}
                                </div>\`;
                            });
...
server.listen(PORT, '0.0.0.0', () => { 
    startGlobalEngine(); 
    sendTG("🚀 *Quantum Power DCA Online!* আপনার যাত্রা সফল হোক।", FIXED_CHAT_ID);
});
