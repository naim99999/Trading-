...
// ==========================================
// 🛡️ সিস্টেম কনফিগ (Quantum Core v6.0 - Aggressive)
// ==========================================
            ...
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 1) {
                ...
                if (sameCoin.length === 0) {
                    ...
                    if (order) {
                        ...
                        sendTG(`🚀 *TRADE START:* #${msg.s}\nEntry: ${buyPrice}\nTarget: ${sellPrice.toFixed(coin.d)}`, config.cid);
                    }
                }
            }
        }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 flex items-center min-h-screen text-center"><div class="max-w-md mx-auto w-full space-y-6">
            <h1 class="text-5xl font-black text-sky-400 italic">Nebula V6</h1>
                ...
                <div class="max-w-xl mx-auto space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-5 bg-slate-900 rounded-[2rem] text-center border border-slate-800">
                            <p class="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Total Growth</p>
                            <p class="text-3xl font-black text-green-400">৳<span id="profitText">0</span></p>
                        </div>
                        <div class="p-5 bg-slate-900 rounded-[2rem] text-center border border-slate-800">
                            <p class="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Completed</p>
                            <p class="text-3xl font-black text-sky-400" id="countText">0</p>
                        </div>
                    </div>
                    <div id="slotContainer" class="space-y-3"></div>
                    <div class="flex gap-3 pt-4">
                        <a href="/reset?id=${userId}" onclick="return confirm('সব ডাটা রিসেট করবেন?')" class="flex-1 bg-red-900/20 border border-red-500/30 text-red-500 py-4 rounded-full text-center text-[10px] font-black">RESET DATA</a>
                        <button onclick="location.reload()" class="flex-1 bg-sky-600 py-4 rounded-full text-[10px] font-black">REFRESH</button>
                    </div>
                </div>
                <script>
