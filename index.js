...
// ==============================================
// 👑 QUANTUM AI MASTER v95.0 - ABSOLUTE BTC SYNC
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = '/tmp/quantum_final_v95.json';
    ...
    ws.on('message', (data) => {
        try {
            ...
            if (market[d.s]) {
                ...
                if (d.s === "BTCUSDT" && s.history.length > 10) {
...
setInterval(async () => {
    for (let uid in cachedUsers) {
        ...
        if (!u.userSlots || u.userSlots.length !== aiSlotsCount) {
            u.userSlots = Array(aiSlotsCount).fill(null).map((_, i) => ({ id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, curP: 0, dca: 0, maxP: 0, totalFeePaid: 0, lastDcaTime: 0, netBDT: 0, pnl: 0 }));
            saveDB();
        }
        ...
        u.userSlots.forEach(async (sl) => {
            ...
            let netUSD = (currentVal - sl.totalCost) - (sl.totalFeePaid + (currentVal * 0.00045));
        ...
        if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < aiSlotsCount && btcP > 0) {
            for (let coin of COINS) {
                ...
                if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                    ...
                    if (parseFloat(qty) > 0 && await placeOrder(coin.s, "BUY", qty, u)) {
                        ...
                        if (sIdx !== -1) {
                            u.userSlots[sIdx] = { id: sIdx, active: true, status: 'ACTIVE', sym: coin.s, buy: m.p, qty: qty, totalCost: (parseFloat(qty) * m.p), marginUsed: mUsed, totalFeePaid: (mUsed * u.lev * 0.0002), curP: m.p, dca: 0, pnl: 0, maxP: m.p, isClosing: false, lastDcaTime: 0, netBDT: 0 };
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        res.end(`<!DOCTYPE html><html><body><h1 style="text-align:center;">QUANTUM MASTER ENGINE</h1><form action="/register" method="GET" style="display:flex;flex-direction:column;max-width:300px;margin:auto;gap:10px;"><input name="id" placeholder="Username" required><select name="mode"><option value="demo">Demo Mode</option><option value="live">Live Trading</option></select><input name="cap" type="number" placeholder="Capital $"><input name="target" type="number" placeholder="Target ৳"><input name="lev" type="number" placeholder="Leverage"><button type="submit">Start</button></form></body></html>`);
    } else {
        ...
        <div class="p-4 bg-slate-900 rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v95 FINAL</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div></div><script>
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '%' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Gain: ৳\${s.netBDT}</div><div class="text-green-400 font-black">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum MASTER</div></div>\` : ''}</div>\`;
