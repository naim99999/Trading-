...
// ==============================================
// 🛡️ Quantum AI Master v2000.0 - THE DIVINE SNIPER
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'nebula_divine_hub.json';
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            let u = cachedUsers[uid]; if (u.status === 'COMPLETED' || u.isPaused) continue;
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                let minThreshold = (Number(u.cap) < 10) ? 0.50 : 1.00;
                let isDropping = sl.netBDT <= (sl.maxNetBDT - 0.01);

                if (sl.netBDT >= minThreshold && isDropping) {
                ...
                if (rawPnL <= dcaT && sl.dca < (u.cap < 10 ? 2 : 4) && (sl.totalCost/u.lev)*2 < u.cap*0.9) {
                    if (await placeOrder(sl.sym, "BUY", sl.qty, u)) {
                        ...
                        sendTG(`🌀 <b>DCA Hit: #${sl.sym}</b>\nAdded: $${stM.toFixed(4)}`, u.cid);
                    }
                }
            });

            if (activeTrades < u.slots && (Number(u.profit) * 124) < Number(u.targetBDT)) {
                for (let sym of Object.keys(market)) {
                    ...
                    if (m.rsi < 32 && m.p < (localHigh * 0.9950) && m.p > (m.low * 1.0008)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (sIdx !== -1 && await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                activeTrades++; saveDB(); sendTG(`🚀 <b>SNIPER ENTRY: #${sym}</b>\nPrice: $${m.p.toFixed(4)}`, u.cid);
                            }
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        res.end(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-[#020617] text-white p-6 flex items-center min-h-screen font-sans text-center"><div class="max-w-md mx-auto w-full space-y-6"><h1 class="text-7xl font-black text-sky-400 italic">QUANTUM</h1><form action="/register" method="GET" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 border border-slate-800 text-left shadow-2xl"><input name="id" placeholder="Username" class="w-full bg-black p-4 rounded-xl outline-none" required><div class="grid grid-cols-2 gap-2"><select name="mode" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><option value="live">Live Trading</option><option value="demo">Demo Mode</option></select><select name="fmode" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><option value="usdt">Fee: USDT</option><option value="bnb">Fee: BNB</option></select></div><input name="api" placeholder="API Key" class="w-full bg-black p-4 rounded-xl outline-none"><input name="sec" placeholder="Secret Key" class="w-full bg-black p-4 rounded-xl outline-none"><input name="cid" placeholder="Chat ID" class="w-full bg-black p-4 rounded-xl outline-none"><div class="grid grid-cols-2 gap-2"><input id="capI" name="cap" type="number" placeholder="Capital $" class="bg-black p-4 rounded-xl outline-none" oninput="sug()"><input name="target" type="number" placeholder="Target ৳" class="bg-black p-4 rounded-xl outline-none"></div><div class="grid grid-cols-2 gap-2"><input name="lev" id="levI" type="number" placeholder="Leverage" class="bg-black p-4 rounded-xl outline-none"><input name="slots" id="slotI" type="number" placeholder="Slots" class="bg-black p-4 rounded-xl outline-none"></div><button type="submit" class="w-full bg-sky-600 p-5 rounded-full font-black uppercase shadow-xl">Launch Hub</button></form></div><script>function sug(){let c=document.getElementById('capI').value; if(c){let l=c<15?20:25; let s=c<25?1:(c<80?2:3); document.getElementById('levI').value=l; document.getElementById('slotI').value=s;}}</script></body></html>`);
    } else {
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
