...
// ==========================================
// 🛡️ Quantum AI - Master Core (For Small Capital)
// ==========================================
...
// ৪০টি সবচেয়ে বেশি মুভমেন্ট করা কয়েন
const COINS = [
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            userSlots[userId].forEach(async (sl) => {
                ...
                if (sl.status === 'BOUGHT') {
                    // PnL calculation including estimated 0.1% round-trip fee
                    sl.pnl = (((s.p - sl.buy) / sl.buy) * 100 * config.lev) - 0.1;

                    // ১. ইনস্ট্যান্ট প্রফিট বুকিং (Market Sell)
                    if (s.p >= sl.sell) {
                        const gain = (sl.qty * s.p) - (sl.qty * sl.buy);
                        sl.active = false; config.profit += (gain * 0.998); 
                        config.count += 1;
                        saveUser(userId, config);
                        sendTG(`💰 *Profit Success:* +৳${(gain*124).toFixed(0)} (#${sl.sym})`, config.cid);
                        sl.status = 'IDLE';
                        if(config.mode !== 'demo') await placeOrder(sl.sym, "SELL", 0, sl.qty, config, "MARKET");
                    }

                    // ২. ইনস্ট্যান্ট স্টপ লস
                    if (s.p <= sl.slP) {
            ...
            // ৩. সুপার অ্যাগ্রেসিভ এন্ট্রি লজিক (Flash Entry)
            ...
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 1) {
                ...
                if (sameCoin.length === 0) {
                    ...
                    const buyPrice = s.p;
                    const sellPrice = buyPrice * 1.0022; // ০.২২% টার্গেট
                    const stopPrice = buyPrice * 0.9920; // ০.৮% এসএল
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 flex items-center min-h-screen"><div class="max-w-md mx-auto w-full space-y-6">
            ...
            <form action="/register" method="GET" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 border border-slate-800">
                ...
                <button type="submit" class="w-full bg-sky-600 p-5 rounded-full font-black uppercase text-xl shadow-lg shadow-sky-600/30">Activate AI</button>
            </form></div></body></html>`);
    } else {
        res.end(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script></head>
            <body class="bg-[#020617] text-white p-4 font-sans uppercase tracking-tight">
                <div class="max-w-xl mx-auto space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="p-6 bg-slate-900 rounded-[2.5rem] text-center border border-slate-800">
                            <p class="text-[9px] text-slate-500 font-bold mb-1">Trades</p>
                            <p class="text-4xl font-black text-sky-400" id="countText">0</p>
                        </div>
                    </div>
                    <div id="slotContainer" class="space-y-3"></div>
                    <div class="flex gap-3 pt-4">
                        <a href="/reset?id=${userId}" onclick="return confirm('Reset Everything?')" class="flex-1 bg-red-900/20 border border-red-500/30 text-red-500 py-5 rounded-full text-center text-xs font-black">RESET</a>
