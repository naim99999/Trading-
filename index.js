...
// 🎯 ৫০টি হাই-ভলিউম কয়েন (স্লট কখনো খালি থাকবে না)
const COINS = [
    ...
    { s: "LINKUSDT", n: "LINK", d: 3, qd: 2 }, { s: "SUIUSDT", n: "SUI", d: 4, qd: 1 }, { s: "APTUSDT", n: "APT", d: 3, qd: 1 },
    { s: "TIAUSDT", n: "TIA", d: 3, qd: 1 }, { s: "ARBUSDT", n: "ARB", d: 4, qd: 1 }, { s: "INJUSDT", n: "INJ", d: 3, qd: 2 },
    { s: "MATICUSDT", n: "MATIC", d: 4, qd: 0 }, { s: "OPUSDT", n: "OP", d: 4, qd: 1 }
];
...
async function startGlobalEngine() {
    ...
    ws.on('message', async (data) => {
        ...
        // ১০-মিনিট রিপোর্ট লজিক (Duplicate Free)
        ...
        for (let userId in allUsers) {
            ...
            if (!isAdmin && config.status !== 'active') continue;
            ...
            // ১. স্লট ট্র্যাকিং (DCA, Sell & Progress)
            slots.forEach(async (sl) => {
                if (!sl.active || sl.sym !== msg.s) return;
                sl.curP = s.p;

                if (sl.status === 'WAITING') {
                    if (Date.now() - sl.waitTime > 120000) { sl.active = false; sl.status = 'IDLE'; sl.sym = ''; return; }
                    if (s.p <= sl.buy) { sl.status = 'BOUGHT'; sendTG(`📥 *বাই সম্পন্ন করা হয়েছে!* (S${sl.id+1})\nকয়েন: *${sl.sym}*\nদাম: ${s.p}`, config.cid); }
                }
            ...
            // ২. রকেট এন্ট্রি (ক্রস-স্লট ও হাই স্পিড)
...
// 🌐 মাস্টার ড্যাশবোর্ড UI (আপনার সব প্রিয় ডিজাইন এলিমেন্টসহ)
            ...
            <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
                <div class="p-6 bg-slate-900 rounded-[2rem] border border-sky-500/40 shadow-xl shadow-sky-500/10">
                    <div class="flex justify-between items-center mb-2">
                        <div><h2 class="text-3xl font-black italic underline decoration-sky-600 underline-offset-8 uppercase">${userId}</h2><p class="text-[9px] ${mColor} font-black uppercase tracking-widest mt-4">${mText} INTENSITY ATTACK</p></div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-6 bg-slate-900 rounded-[2rem] border border-slate-800 relative overflow-hidden shadow-2xl"><p class="text-[10px] text-slate-500 uppercase font-black">TOTAL PROFIT (BDT)</p><p class="text-4xl font-bold text-green-400 mt-2">৳${(user.profit * 124).toFixed(0)}</p><p class="text-xs text-slate-600 mt-1 italic uppercase tracking-tighter">Life Time Gain</p><div class="card-icon">💲</div></div>
                    <div class="p-6 bg-slate-900 rounded-[2rem] border border-slate-800 relative overflow-hidden shadow-2xl"><p class="text-[10px] text-slate-500 uppercase font-black">SUCCESS TRADES</p><p class="text-4xl font-bold text-sky-400 mt-2">${user.count}</p><p class="text-xs text-slate-600 mt-1 italic uppercase tracking-tighter">Approximate Value</p><div class="card-icon">💼</div></div>
                </div>
