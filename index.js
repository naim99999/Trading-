...
// ==========================================
// 🛡️ Quantum AI - Ultimate Zero-Loss v30.0
// ==========================================
...
const COINS = [
    ...
    { s: "SHIBUSDT", n: "SHIB", d: 8, qd: 0 }, { s: "SATSUSDT", n: "SATS", d: 7, qd: 0 },
    { s: "FLOKIUSDT", n: "FLOKI", d: 6, qd: 0 }, { s: "JUPUSDT", n: "JUP", d: 4, qd: 1 },
    { s: "ICPUSDT", n: "ICP", d: 3, qd: 1 }, { s: "TIAUSDT", n: "TIA", d: 4, qd: 1 }
];
...
// ১. পাওয়ারফুল টেলিগ্রাম রিপোর্টার (Fix)
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            userSlots[userId].forEach(async (sl) => {
                ...
                // স্মার্ট DCA (২% ডাউনে গড় ক্রয়মূল্য কমানো)
                if (sl.dca < 3 && rawPnL <= -2.0) {
                    ...
                    if (order) {
                        ...
                        sendTG(`🌀 *DCA Executed:* #${sl.sym} এভারেজ করা হয়েছে। নতুন এন্ট্রি: ${sl.buy.toFixed(4)}`, config.cid);
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/test-tg') {
        const uid = url.searchParams.get('id');
        const success = await sendTG("🔔 *Test:* আপনার টেলিগ্রাম আপডেট এখন চালু আছে!", db[uid]?.cid);
        ...
        res.end(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script></head>
            <body class="bg-[#020617] text-white p-4 font-sans uppercase">
                <div class="max-w-xl mx-auto space-y-4">
                    <div class="p-6 bg-slate-900 rounded-[2.5rem] border-2 border-sky-500/50 text-center">
                        <p class="text-[10px] text-sky-400 font-bold mb-1 uppercase tracking-widest">Binance Futures Balance</p>
                        <p class="text-5xl font-black text-white">$<span id="balanceText">0.00</span></p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-5 bg-slate-900 rounded-[2rem] text-center border border-slate-800"><p class="text-[9px] text-slate-500 font-bold mb-1">Growth (BDT)</p><p class="text-4xl font-black text-green-400">৳<span id="profitText">0</span></p></div>
                        <div class="p-5 bg-slate-900 rounded-[2.5rem] text-center border border-slate-800"><p class="text-[9px] text-slate-500 font-bold mb-1">Wins</p><p class="text-4xl font-black text-sky-400" id="countText">0</p></div>
                    </div>
                    ...
                    <div class="space-y-3 pt-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <a href="/reset?id=${userId}" onclick="return confirm('রিসেট করবেন?')" class="bg-red-900/20 border border-red-500/30 text-red-500 py-5 rounded-full text-center text-[10px] font-black">RESET DATA</a>
                        </div>
...
server.listen(PORT, '0.0.0.0', () => { 
    startGlobalEngine(); 
    sendTG("🚀 *Final Quantum Core v30.0 Online!* আপনার যাত্রা সফল হোক।", FIXED_CHAT_ID);
});
