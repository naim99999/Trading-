...
// ==============================================
// 👑 QUANTUM APEX AI v38.0 - LIVE PRODUCTION READY
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_production_v38.json';
...
async function getBinanceBalance(u) {
    if (u.mode === 'demo') return Number(u.cap || 0).toFixed(2);
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (isTargetDone && !u.isPaused) {
                ...
                sendTG(`🎯 <b>TARGET REACHED!</b>\nFinal Profit: ৳${(u.profit*124).toFixed(2)}\nBot Paused.`, u.cid);
            }

            let aiSlotsCount = walletBal < 30 ? 2 : (walletBal < 150 ? 4 : 8);
            if (!u.userSlots || u.userSlots.length !== aiSlotsCount) {
                u.userSlots = Array(aiSlotsCount).fill(null).map((_, i) => ({ id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, orderId: null, entryTime: 0 }));
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                if (sl.status === 'BUY_PENDING') {
                    ...
                    if (ms.p <= sl.buy || u.mode === 'demo') {
                        ...
                        if (sellOrder) {
                            sl.targetP = targetPrice; sl.orderId = sellOrder.orderId;
                            sl.status = 'SELL_PENDING'; saveDB();
                            sendTG(`🚀 <b>BUY FILLED: #${sl.sym}</b>\nPrice: ${sl.buy}\nTarget: ${targetPrice}`, u.cid);
                        }
                    } else if (waitSec > 60) { // ১ মিনিট পর অটো ক্যানসেল
                        ...
                        sendTG(`⏳ <b>LIMIT EXPIRED: #${sl.sym}</b>`, u.cid);
                    }
                    return;
                }
                ...
                if (sl.status === 'SELL_PENDING') {
                    if (ms.p >= sl.targetP || u.mode === 'demo') {
                        ...
                        sendTG(`✅ <b>PROFIT! #${sl.sym}</b>\nGain: ৳${(netUSD*124).toFixed(2)}\nWallet: $${u.cap.toFixed(2)}`, u.cid);
                        Object.assign(sl, { active: false, status: 'IDLE', sym: '', orderId: null, totalCost: 0 }); saveDB();
                    }
                    return;
                }
            });
            ...
            if (!u.isPaused && !isTargetDone && u.userSlots.filter(s => s.active).length < (aiSlotsCount - 1)) {
                for (let sym of Object.keys(market)) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === sym)) {
                        ...
                        let qty = (entryVal / limitPrice).toFixed(COINS.find(c => c.s === sym).qd);
                        ...
                        if (sIdx !== -1 && walletBal > (mNeeded + 2)) {
                            ...
                            if (buyOrder) {
                                ...
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: sym, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: mNeeded, entryTime: Date.now(), orderId: buyOrder.orderId, targetP: 0 };
                                saveDB();
                            }
                        }
                    }
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/data') {
        ...
        return res.end(JSON.stringify({ ...u, balance: Number(u?.cap || 0).toFixed(2), btcPrice: btc.p.toFixed(2), btcTrend: btc.btcTrend.toFixed(2) }));
    }
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">AI Status</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v38</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' ['+s.status+']' : 'Slot '+(i+1)+' Idle'}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Goal: \${(s.targetP || 0).toFixed(4)}</div><div class="text-right italic">Production READY</div></div>\` : ''}</div>\`;
