...
// ==============================================
// 🛡️ Quantum AI Master v1000.4 - FULL AI CONTROL
// ==============================================
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            // --- 🤖 AI AUTO OVERRIDE (পুস/অন কন্ট্রোল) ---
            if (btcT < -0.10) { 
                if (!u.isPaused) { u.isPaused = true; saveDB(); sendTG("⚠️ <b>AI AUTO-PAUSE:</b> Market dropping. Entries secured.", u.cid); }
            } else if (btcT > 0.03) { 
                if (u.isPaused) { u.isPaused = false; saveDB(); sendTG("🚀 <b>AI AUTO-RESUME:</b> Trend recovery detected. Hunter active.", u.cid); }
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                let minP = 0.50; // সাধারণ সময়ে কমপক্ষে ৫০ পয়সা লাভ
                if (u.isPaused || btcT < 0) minP = 0.15; // মার্কেট দুর্বল থাকলে ১৫ পয়সাতেই প্রস্থান
                if (targetReached) minP = 0.01; // টার্গেট ফিলাপ হলে ১ পয়সা লাভেই প্রস্থান

                // ট্রেইলিং মেকানিজম: দাম ১ পয়সা কমলেই অটো-সেল
                let dropTrigger = sl.maxNetBDT - 0.01;

                if (sl.netBDT >= minP && (sl.netBDT <= dropTrigger || u.isPaused || targetReached)) {
            ...
            // --- 🎯 AI HUNTER (ভুল সময়ে এন্ট্রি বন্ধ রাখা) ---
            if (!u.isPaused && !targetReached && activeTrades < u.slots) {
                // বিটকয়েন যদি নিচের দিকে ঝুঁকে থাকে তবে এন্ট্রি হবে না
                if (btcT < -0.05) continue; 

                for (let sym of Object.keys(market)) {
                    if (activeTrades >= u.slots) break;
                    const m = market[sym]; if (m.p === 0 || m.history.length < 20) continue;
                    
                    // এন্ট্রি ফিল্টার: RSI এবং বিটকয়েন মোমেন্টাম
                    if (m.rsi < 45 && m.p < (Math.max(...m.history) * 0.9982)) {
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">BTC AI OVERRIDE</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div id="aiStat" class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ AI MASTER CONTROL</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">AI Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; d.userSlots.forEach((s, i) => {
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${s.active ? 'text-sky-400' : 'text-zinc-700'} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span>\${s.active ? \`<span class="text-[11px] font-black \${s.netBDT>=0?'text-green-500':'text-red-400'}">৳\${s.netBDT.toFixed(2)}</span>\` : ''}</div>\${s.active ? \`<div class="w-full bg-black h-1.5 rounded-full overflow-hidden mb-4"><div class="h-full bg-sky-500 transition-all duration-1000" style="width: 50%"></div></div><div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right">Live: \${s.curP.toFixed(4)}</div><div class="text-indigo-400 italic text-center col-span-2 mt-1">\${d.isPaused ? 'AI Secure Mode' : 'AI Sniper Active'}</div></div>\` : ''}</div>\`;
