...
// ==============================================
// 👑 QUANTUM master engine v57.0 - VELOCITY SCALPER
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_ai_v57_velocity.json';
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                ...
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => existing.find(x => x.id === i) || { id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, targetP: 0, curP: 0, dca: 0, maxNetBDT: 0, entryTime: 0 });
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                let netUSD = (tradeVal - sl.totalCost) - estFee;
                ...
                if (sl.status === 'SELL_PENDING') {
                    ...
                    // ⚡ VELOCITY EXIT LOGIC: টার্গেটের ৮০% হিট করে নামা শুরু করলেই সেল
                    let speedThreshold = sl.targetP * 0.9998; 
                    let reversing = (sl.maxNetBDT - parseFloat(sl.netBDT)) > 0.03;

                    if ((ms.p >= sl.targetP || (ms.p >= speedThreshold && reversing)) && parseFloat(sl.netBDT) >= 0.30) {
                        let gainText = ms.p >= sl.targetP ? "✅ <b>GOAL HIT!</b>" : "⚡ <b>FAST EXIT!</b>";
                        ...
                        sendTG(`${gainText} <b>#${sl.sym}</b>\nGain: <b>৳${sl.netBDT}</b>\nTotal: ৳${(u.profit * 124).toFixed(2)}`, u.cid);
                        Object.assign(sl, { active: false, status: 'IDLE', sym: '', marginUsed: 0, totalCost: 0, dca: 0, maxNetBDT: 0 }); saveDB();
                    } 
                    // 🌀 RECOVERY DCA
                    else if (parseFloat(sl.pnl) < -3.0 && sl.dca < 6 && walletBal > (sl.marginUsed * 1.5)) {
                        if (ms.p > ms.ema7) {
                            ...
                            sl.marginUsed += addM; sl.dca++; sl.maxNetBDT = 0;
                            sl.targetP = (sl.buy * (1 + 0.005 + feeR)).toFixed(COINS.find(c => c.s === sl.sym).d);
                ...
                for (let coin of COINS) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        ...
                        if (sIdx !== -1) {
                            ...
                            u.userSlots[sIdx] = { id: sIdx, active: true, status: 'BUY_PENDING', sym: coin.s, buy: parseFloat(limitPrice), qty: qty, totalCost: (parseFloat(qty) * parseFloat(limitPrice)), marginUsed: mNeeded, entryTime: Date.now(), curP: m.p, dca: 0, pnl: 0, maxNetBDT: 0 };
                            saveDB();
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">Velocity AI Status</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ QUANTUM v57 PRO</div></div></div>
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4 uppercase"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/30 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
            ...
            async function updateData() { try { const res = await fetch('/api/data?id=${userId}'); const d = await res.json(); 
                ...
                let h = ''; (d.userSlots || []).forEach((s, i) => { 
                    ...
                    h += \`<div class="p-5 bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-zinc-800 mb-3 shadow-lg uppercase"><div class="flex justify-between items-center mb-3"><span class="text-[11px] font-black \${statusColor} tracking-wider">\${s.active ? s.sym + ' [DCA:'+s.dca+']' : 'Slot '+(i+1)+' Idle'}</span><span class="text-[11px] font-black \${pnlColor}">\${s.active ? s.pnl + '% ('+bdtVal+')' : ''}</span></div>\${s.active ? \`<div class="grid grid-cols-2 text-[10px] font-mono text-slate-500 gap-y-1"><div>Buy: \${s.buy.toFixed(4)}</div><div class="text-right text-indigo-400 font-bold">Goal: \${(s.targetP || 0).toFixed(4)}</div><div class="text-green-400 font-black">Live: \${(s.curP || s.buy).toFixed(4)}</div><div class="text-right italic">Quantum v57</div></div>\` : ''}</div>\`;
