...
// ==============================================
// 🛡️ Quantum AI Master v1200.0 - THE RESURRECTOR
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'nebula_resurrector_hub.json';
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            let activeCount = activeTrades.length;
            let idleSlotsCount = u.slots - activeCount;
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // ০.০১ টাকা ট্রেইলিং লক লজিক
                if (sl.netBDT > (sl.maxNetBDT || 0)) sl.maxNetBDT = sl.netBDT;
                let minP = u.isPaused ? 0.15 : (Number(u.cap) < 10 ? 0.50 : 1.00);
                let dropGap = sl.maxNetBDT - 0.01;

                if (sl.netBDT >= minP && (u.isPaused || (sl.maxNetBDT > 0 && sl.netBDT <= dropGap))) {
                    ...
                    sendTG(`✅ <b>RECOVERED: #${sl.sym}</b>\nProfit: ৳${sl.netBDT.toFixed(2)}\nGrowth: ৳${(u.profit * 124).toFixed(0)}`, u.cid);
                ...
                // --- টিমওয়ার্ক রিকভারি DCA (আপনার নতুন আইডিয়া) ---
                let dcaT = sl.dca === 0 ? -1.8 : -4.5;
                // যদি অন্য স্লট অলস থাকে, তবে পুঁজি ব্যবহার করে সেধে DCA করবে (-০.৭% হলেই)
                if (idleSlotsCount >= 1 && rawPnL <= -0.7 && sl.dca < 3) dcaT = -0.7; 
                ...
                if (rawPnL <= dcaT && sl.dca < (u.cap < 15 ? 2 : 4) && (sl.totalCost/u.lev)*2 < (u.cap * 0.92)) {
                    if (await placeOrder(sl.sym, "BUY", sl.qty, u)) {
                        ...
                        sl.buy = sl.totalCost / parseFloat(sl.qty); sl.dca++; sl.sell = sl.buy * 1.0035; sl.be = false; sl.maxNetBDT = 0; saveDB();
                        sendTG(`🌀 <b>TEAM RESCUE: #${sl.sym} (L${sl.dca})</b>\nAdded Budget: $${stM.toFixed(4)}`, u.cid);
                    }
                }
            });

            // এন্ট্রি লজিক (নিখুঁত তলা ডিটেকশন)
            let anyDanger = u.userSlots.some(s => s.active && s.dca >= 3);
            if (!u.isPaused && activeCount < u.slots && !anyDanger && (Number(u.profit) * 124) < Number(u.targetBDT)) {
                for (let sym of Object.keys(market)) {
                    if (activeCount >= u.slots) break; 
                    if (sym === "GALAUSDT" && Number(u.cap) < 50) continue;
                    const m = market[sym]; if (m.p === 0 || m.history.length < 20 || (u.cooldowns[sym] && Date.now() < u.cooldowns[sym])) continue;
                    
                    // Sniper Logic: RSI 30 + Dip 0.50% + 0.08% বাউন্স
                    const localHigh = Math.max(...m.history);
                    if (m.rsi < 35 && m.p < (localHigh * 0.9950) && m.p > (m.low * 1.0008)) {
                        if (!u.userSlots.some(x => x.active && x.sym === sym)) {
                            ...
                            if (sIdx !== -1 && await placeOrder(sym, "BUY", qty, u)) {
                                ...
                                u.userSlots[sIdx] = { id: sIdx, active: true, status: 'TRADING', sym: sym, buy: m.p, sell: m.p * 1.0040, slP: 0, qty: qty, pnl: 0, curP: m.p, dca: 0, totalCost: (parseFloat(qty) * m.p), be: false, netBDT: -0.10, isClosing: false, maxNetBDT: 0 };
                                activeCount++; saveDB(); sendTG(`🚀 <b>BOTTOM ENTRY: #${sym}</b>`, u.cid);
                            }
                        }
                    }
                }
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        res.end(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-[#020617] text-white p-6 flex items-center min-h-screen font-sans text-center"><div class="max-w-md mx-auto w-full space-y-6"><h1 class="text-7xl font-black text-sky-400 italic">QUANTUM</h1><form action="/register" method="GET" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 border border-slate-800 text-left shadow-2xl"><input name="id" placeholder="Username" class="w-full bg-black p-4 rounded-xl outline-none" required><div class="grid grid-cols-2 gap-2"><select name="mode" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><option value="live">Live Trading</option><option value="demo">Demo Mode</option></select><select name="fmode" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><option value="usdt">Fee: USDT</option><option value="bnb">Fee: BNB</option></select></div><input name="api" placeholder="Binance API Key" class="w-full bg-black p-4 rounded-xl outline-none"><input name="sec" placeholder="Binance Secret" class="w-full bg-black p-4 rounded-xl outline-none"><input name="cid" placeholder="Chat ID" class="w-full bg-black p-4 rounded-xl outline-none"><div class="grid grid-cols-2 gap-2"><input id="capI" name="cap" type="number" placeholder="Capital $" class="bg-black p-4 rounded-xl outline-none" oninput="sug()"><input name="target" type="number" placeholder="Target ৳" class="bg-black p-4 rounded-xl outline-none"></div><div class="grid grid-cols-2 gap-2"><input name="lev" id="levI" type="number" placeholder="Leverage" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"><input name="slots" id="slotI" type="number" placeholder="Slots" class="bg-black p-4 rounded-xl border border-slate-800 outline-none"></div><button type="submit" class="w-full bg-sky-600 p-5 rounded-full font-black uppercase shadow-xl">Initialize Master Hub</button></form></div><script>function sug(){let c=document.getElementById('capI').value; if(c){let l=c<15?20:25; let s=c<25?1:(c<80?2:3); document.getElementById('levI').value=l; document.getElementById('slotI').value=s;}}</script></body></html>`);
    } else {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mt-1"><div><p class="text-[8px] text-slate-500 font-bold">BTC Market Pulse</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ RESURRECTOR MODE</div></div></div>
        <div class="p-6 bg-slate-900 rounded-[2.5rem] border-2 border-sky-500/50 text-center shadow-2xl tracking-tighter"><p class="text-[10px] text-sky-400 font-bold mb-1 italic">Silent Wallet Balance</p><p class="text-5xl font-black text-white">$<span id="balanceText">0.00</span></p></div>
