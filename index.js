...
// ==============================================
// 🛡️ Quantum AI Master v1000.0 - THE FINAL APEX
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'nebula_apex_final.json';
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                // আল্ট্রা-ফাস্ট ট্রেইলিং লকিং (১১২ বনাম ৮৬ সমস্যার সমাধান)
                if (sl.netBDT > (sl.maxNetBDT || 0)) {
                    sl.maxNetBDT = sl.netBDT;
                    // লাভ যত বাড়বে, লক তত টাইট হবে (৳০.০১ থেকে ৳০.০০৫ গ্যাপ)
                    let gap = sl.netBDT > 100 ? 0.005 : 0.01;
                    sl.slP = rawPnL - (gap / 124 / sl.totalCost * u.lev); 
                    sl.be = true;
                }

                // ডাইনামিক রিকভারি DCA
                let dcaT = sl.dca === 0 ? -1.8 : -4.5;
                let idleSlots = u.slots - activeTrades;
                if (idleSlots >= 1 && rawPnL <= -0.7 && sl.dca < 2) dcaT = -0.7; 

                if (rawPnL <= dcaT && sl.dca < (u.cap < 20 ? 2 : 4) && (sl.totalCost/u.lev)*2 < u.cap*0.95) {
            ...
            // অ্যান্টি-ক্রাশ এন্ট্রি (১টি স্লট বিপদজনক লসে থাকলে অন্যটি নতুন ট্রেড নিবে না)
            let anyDanger = u.userSlots.some(s => s.active && s.dca >= 2);
            let currentTotalBDT = Number(u.profit || 0) * 124;

            if (!u.isPaused && activeTrades < u.slots && !anyDanger && currentTotalBDT < Number(u.targetBDT)) {
                for (let sym of Object.keys(market)) {
                    ...
                    if (m.rsi < 45 && m.p < (Math.max(...m.history) * 0.9990) && m.p > (m.low * 1.0001)) {
...
const server = http.createServer(async (req, res) => {
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div id="slotContainer" class="space-y-3"></div><div class="grid grid-cols-2 gap-3 pt-4"><button onclick="togglePause()" id="pauseBtn" class="py-5 rounded-full text-[10px] font-black bg-orange-900/20 border border-orange-500/50 text-orange-400">Pause</button><a href="/reset-logout?id=${userId}" class="block bg-slate-800 border border-slate-700 text-slate-400 py-5 rounded-full text-center text-[10px] font-black uppercase">Logout</a></div></div><script>
