...
// ==========================================
// 🛡️ মাস্টার এডমিন সেটিংস
// ==========================================
const ADMIN_USER = "naim1155"; 
const ADMIN_PASS = "115510"; 
const ADMIN_TG_TOKEN = "8281887575:AAGRTPvSdT4ho8C2NwsxCHyUMkRq2q6qWDc"; // আপনার মাস্টার টোকেন
...
const DB_FILE = 'nebula_v15_db.json';
const CONFIG = {
    netFee: 0.0008, // এন্ট্রি + এক্সিট ফি
    bdt: 124, 
...
async function sendTG(msg, chatId) {
    ...
    } catch (e) { console.error("❌ TG Error: User might not have started the bot."); }
}
...
async function placeOrder(symbol, side, price, qty, config, type = "LIMIT") {
    ...
    try {
        const res = await axios.post(`${CONFIG.baseUrl}/fapi/v1/order?${query}&signature=${signature}`, null, {
        ...
        for (let userId in allUsers) {
                ...
                if (sl.status === 'BOUGHT') {
                    ...
                    if (drop >= 0.45 && sl.dca < 12) {
                        ...
                        if (order) {
                            ...
                            sendTG(`🛡️ *DCA রিকাভারি সচল!* \nকয়েন: ${sl.sym} | লেয়ার: ${sl.dca}`, config.cid);
                        }
                    }
            ...
            if (!config.isPaused && slotIdx !== -1 && s.trend >= 2 && s.p < avgP) {
                ...
                if (canBuy) {
                    ...
                    // ক্যাপিটাল এবং রেভারেজ অনুযায়ী ডাইনামিক Quantity হিসাব
                    const marginPerSlot = config.cap / 5;
                    const qty = ((marginPerSlot * config.lev) / parseFloat(buyP)).toFixed(COINS.find(c=>c.s===msg.s).qd);
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen text-center"><div class="max-w-md mx-auto space-y-6 w-full">
            ...
            <form action="/register" class="bg-slate-900 p-8 rounded-[2.5rem] space-y-4 text-left shadow-2xl">
                ...
                <input name="api" placeholder="API Key" class="w-full bg-black p-4 rounded-2xl border border-slate-800"><input name="sec" placeholder="Secret Key" class="w-full bg-black p-4 rounded-2xl border border-slate-800">
        ...
        <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
            <div class="p-6 bg-slate-900 rounded-[2rem] border border-sky-500/40 shadow-xl flex justify-between items-center">
                <div><h2 class="text-3xl font-black italic underline decoration-sky-600 uppercase">${userId}</h2><p class="text-[9px] ${mColor} font-black uppercase tracking-widest mt-4 animate-pulse">${mText} INTENSITY</p></div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="p-6 bg-slate-900 rounded-[2rem] border border-slate-800 relative overflow-hidden shadow-2xl"><p class="text-[10px] text-slate-500 uppercase font-black">TOTAL PROFIT</p><p class="text-4xl font-bold text-green-400 mt-2">$${user.profit.toFixed(2)}</p><div class="card-icon">💲</div></div>
            ...
            <div class="p-6 bg-zinc-900/50 rounded-[2.5rem] border border-zinc-800 space-y-3 shadow-inner">
                ${slots.map((s,i) => {
                    ...
                    return `<div class="p-4 bg-black/40 rounded-2xl border border-zinc-800/50 flex justify-between items-center"><div><span class="text-[9px] font-bold text-slate-600 italic uppercase">Slot ${i+1}</span><p class="text-sm font-black ${s.active ? 'text-sky-400' : 'text-zinc-800'}">${s.active ? s.sym.replace('USDT','') : 'IDLE'}</p></div><div class="text-right">${s.active ? `<span class="text-xs font-bold ${s.pnl>=0?'text-green-500':'text-red-400'}">${s.pnl.toFixed(2)}% PNL</span>` : ''}</div></div>${s.active && s.status === 'BOUGHT' ? `<div class="progress-bar"><div class="progress-fill" style="width: ${progress}%"></div></div>` : ''}`;
                }).join('')}
            </div>
