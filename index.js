...
// ==========================================
// 🛡️ অ্যাডমিন মাস্টার কনফিগ (Oracle Preset)
// ==========================================
...
const DB_FILE = 'master_oracle_db.json';
...
// 🎯 ৫২টি টপ ভলিউম কয়েন (ক্যাপিটাল ১ সেকেন্ডও বসে থাকবে না)
const COINS = [
    ...
    { s: "TIAUSDT", n: "TIA", d: 3, qd: 1 }, { s: "OPUSDT", n: "OP", d: 4, qd: 1 }, { s: "FTMUSDT", n: "FTM", d: 4, qd: 0 },
    { s: "ARBUSDT", n: "ARB", d: 4, qd: 1 }, { s: "INJUSDT", n: "INJ", d: 3, qd: 2 }, { s: "FILUSDT", n: "FIL", d: 3, qd: 1 }
];

let market = {};
COINS.forEach(c => market[c.s] = { p: 0, lp: 0, trend: 0, history: [], mom: 0, waitP: 0 });
let userSlots = {}; 
let lastReportMin = -1;
    ...
    ws.on('message', async (data) => {
        ...
        for (let userId in allUsers) {
            ...
            slots.forEach(async (sl) => {
                ...
                // ২-মিনিট ওয়েটিং চেক (Cancel & Rotate)
                ...
                if (sl.status === 'WAITING' && s.p <= sl.buy) {
                    sl.status = 'BOUGHT';
                    sendTG(`📥 *বাই সম্পন্ন:* (S${sl.id+1}) *${sl.sym}*\nদাম: ${s.p}`, config.cid);
                }
            ...
            // ২-টিক এন্ট্রি (Oracle logic: Buy low, confirm up)
...
// 🌐 মাস্টার ড্যাশবোর্ড UI (আপনার পাঠানো স্ক্রিনশটের ডিজাইনে)
const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/reset-now') {
        if(db[userId]) { 
            db[userId].profit = 0; db[userId].count = 0; 
    ...
    if (!userId || !db[userId]) {
        ...
        <body class="bg-[#020617] text-white p-6 font-sans flex items-center min-h-screen text-center"><div class="max-w-md mx-auto space-y-6 w-full">
            <h1 class="text-5xl font-black text-sky-400 italic underline decoration-sky-600 underline-offset-8">QUANTUM MASTER</h1>
        ...
        getBinanceBalance(user).then(balance => {
            ...
            <body class="p-4 font-sans"><div class="max-w-xl mx-auto space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-6 bg-slate-900 rounded-[2.5rem] border border-slate-800 relative overflow-hidden shadow-2xl"><p class="text-[10px] text-slate-500 uppercase font-black">TOTAL PROFIT (BDT)</p><p class="text-4xl font-bold text-green-400 mt-2">৳${(user.profit * 124).toFixed(0)}</p><div class="card-icon">💲</div></div>
                    <div class="p-6 bg-slate-900 rounded-[2.5rem] border border-slate-800 relative overflow-hidden shadow-2xl"><p class="text-[10px] text-slate-500 uppercase font-black">SUCCESS TRADES</p><p class="text-4xl font-bold text-sky-400 mt-2">${user.count}</p><div class="card-icon">💼</div></div>
                </div>
...
server.listen(PORT, '0.0.0.0', () => {
    console.log("🌐 Portal Live on port " + PORT);
    startGlobalEngine();
});
