...
// ==============================================
// 👑 QUANTUM AI MASTER v84.0 - THE FINAL DESTINY
// ==============================================
const MASTER_TG_TOKEN = "8281887575:AAG5OR86LCQO_90479FKkia2F1sEAJjCP60"; 
const FIXED_CHAT_ID = "5279510350"; 
const DB_FILE = 'quantum_final_destiny.json';
...
async function startGlobalEngine() {
    ...
    setInterval(async () => {
        for (let uid in cachedUsers) {
            ...
            if (isTargetDone && !u.isPaused) { u.isPaused = true; saveDB(); sendTG(`🎯 <b>TARGET REACHED!</b>\nFinal Bal: $${u.cap}`, u.cid); }

            let aiSlots = walletBal < 35 ? 1 : (walletBal < 200 ? 3 : 5);
            if (!u.userSlots || u.userSlots.length !== aiSlots) {
                u.userSlots = Array(aiSlots).fill(null).map((_, i) => ({ id: i, active: false, status: 'IDLE', sym: '', buy: 0, qty: 0, totalCost: 0, marginUsed: 0, dca: 0, maxP: 0, lastDcaTime: 0 }));
                saveDB();
            }
            ...
            u.userSlots.forEach(async (sl) => {
                ...
                let netUSD = (currentVal - sl.totalCost) - (currentVal * feeR * 2);
                ...
                if (sl.status === 'ACTIVE') {
                    if (parseFloat(sl.netBDT) >= 0.50) {
                        ...
                        if (dropFromPeak > 0.10 || sl.pnl > 8.0) {
                            sl.isClosing = true;
                            if (await placeOrder(sl.sym, "SELL", sl.qty, u)) {
                                ...
                                Object.assign(sl, { active: false, status: 'IDLE', sym: '', marginUsed: 0, totalCost: 0, dca: 0, maxP: 0 }); saveDB();
                            } else sl.isClosing = false;
                        }
                    }
                    else if (parseFloat(sl.pnl) < -5.0 && sl.dca < 5 && (Date.now() - sl.lastDcaTime) > 300000 && walletBal > (sl.marginUsed * 2)) {
                        if (ms.p > ms.ema7) {
                ...
                for (let coin of COINS) {
                    ...
                    if (m.rsi < 28 && m.p > m.ema7 && !u.userSlots.some(x => x.active && x.sym === coin.s)) {
                        ...
                        if (await placeOrder(coin.s, "BUY", qty, u)) {
                            ...
                            u.userSlots[sIdx] = { id: sIdx, active: true, status: 'ACTIVE', sym: coin.s, buy: m.p, qty: qty, totalCost: (parseFloat(qty) * m.p), marginUsed: mUsed, curP: m.p, dca: 0, pnl: 0, maxP: m.p, isClosing: false, lastDcaTime: Date.now() };
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/data') {
        ...
        return res.end(JSON.stringify({ ...u, balance: apiData.bal, btcPrice: btc.p.toFixed(2), btcTrend: btc.btcTrend.toFixed(2), pulse: pulse }));
    }
    ...
    if (!userId || !cachedUsers[userId]) {
        ...
        <div class="p-4 bg-slate-900/50 backdrop-blur-md rounded-[2rem] border border-slate-800 shadow-lg relative overflow-hidden"><div id="pB" class="absolute top-0 left-0 h-1 transition-all duration-1000"></div><div class="flex justify-between items-center mb-3"><div><p class="text-[8px] text-slate-500 font-bold">AI Status Meter</p><p class="text-[10px] font-black" id="pM">Syncing...</p><p class="text-[8px] text-slate-400" id="pP">BTC: $0.00</p></div><div class="px-3 py-2 bg-indigo-600/20 border border-indigo-500/50 rounded-lg text-[8px] font-black text-indigo-400">🛡️ MASTER v84 FINAL</div></div></div>
